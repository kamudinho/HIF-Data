...
# --- KONFIGURATION ---
st.set_page_config(layout="wide", page_title="HIF Data Center") # Sikrer bredt layout
...
def vis_side():
    # --- SIDEBAR ---
    with st.sidebar:
        st.image("https://hvidovrefodbold.dk/wp-content/uploads/2021/05/Hvidovre_IF_logo.png", width=100)
        st.title("Indstillinger")
        st.divider()
        st.session_state.form_skygge = st.radio("Vælg Formation", ["3-4-3", "4-3-3", "3-5-2"], index=0)
        st.divider()
        if st.button("🔄 Opdater Data"):
            st.rerun()
        st.caption("Status: Forbundet til GitHub")
    ...
    for t, d, s, p in [(t1, df_emner, e_s, EMNE_PATH), (t2, df_hif, h_s, HIF_PATH)]:
        with t:
            ...
            if not ed['Skyggehold'].equals(d['Skyggehold']) or not ed['POS'].equals(d['POS']):
                ...
                # Sørg for at alle taktiske felter også gemmes som tal
                for c in ['POS_343', 'POS_433', 'POS_352']:
                    d_save[c] = d_save[c].map(REVERSE_POS).fillna(d_save[c])
                push_to_github(p, "Update Selection", d_save.drop(columns=['Klub'], errors='ignore').to_csv(index=False), s)
                st.rerun()
        ...
        if not df_s.empty:
            ...
            for pid, (x, y, lbl) in m.items():
                ...
                for i, (_, p) in enumerate(players.iterrows()):
                    ...
                    ax.text(x, y+(i*4), p['Navn'], size=8, ha='center', weight='bold', bbox=dict(facecolor=bg, edgecolor='#333', alpha=0.9, boxstyle='square,pad=0.1'))
            st.pyplot(fig)
