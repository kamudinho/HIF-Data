...
st.markdown("""
    <style>
        /* Optimeret visning af indholdet */
        .block-container { 
            padding-top: 2rem !important; 
            padding-bottom: 0rem !important; 
        }

        /* MENU-IKONET ER NU SYNLIGT IGEN (Linjerne herunder er fjernet eller ændret) */
        [data-testid="stHeader"] { 
            background-color: rgba(0,0,0,0); /* Gennemsigtig i stedet for display:none */
        }
...
def load_full_data():
    try:
        ...
        # SIKRER AT NAVNE FRA 'SPILLERE' ARK ER KOBLET PÅ EVENTS
        if 'PLAYER_WYID' in events.columns and 'PLAYER_WYID' in spillere.columns:
            # Vi tager kun de nødvendige kolonner fra spillere for at undgå rod
...
with st.sidebar:
    ...
    selected = option_menu(
        menu_title=None,
        options=["DATA - HOLD", "DATA - SPILLERE", "STATISTIK", "SCOUTING"],
...
if selected == "DATA - HOLD":
    ...
    if selected_sub == "Zoneinddeling":
        player_goalzone.vis_side(df_events, spillere)
    elif selected_sub == "Afslutninger":
        st.title("Spiller-afslutninger")
        st.info("Denne funktion er ved at blive klargjort i shots.py")
