...
def vis_profil(p_data, full_df, s_df):
    ...
    # Top sektion: Billede til VENSTRE (head_col1), tekst til HØJRE (head_col2)
...
def vis_side():
    ...
    # Gruppér for seneste status pr. spiller
    ...
    # Definér kolonner til visning (Inkluderer Scout hvis fundet)
    vis_cols = [c_navn, c_pos, c_klub, c_rating, c_status, c_dato]
    if c_scout:
        vis_cols.append(c_scout)
    
    # Tabelvisning med fast højde (Fjerner scroll-problemet)
    event = st.dataframe(
        f_df[vis_cols],
