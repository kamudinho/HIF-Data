...
def hent_vaerdi_robust(row, col_name):
    """Tjekker kolonnenavne uanset store/små bogstaver og mellemrum."""
    # Lav en ordbog hvor alle keys er lowercase og uden mellemrum
    row_clean = {str(k).strip().lower(): v for k, v in row.items()}
    target = str(col_name).strip().lower()
    return row_clean.get(target, 0)
...
def vis_profil(p_data, full_df, s_df):
    st.markdown(f"""
        <div style='text-align: center;'>
            ...
            <p style='font-size: 18px; color: gray;'>{p_data.get('KLUB', '')} | {p_data.get('POSITION', '')} | Snit: {p_data.get('RATING_AVG', 0)}</p>
        </div>
    """, unsafe_allow_html=True)
    
    # Robust historik match
    ...
    with tab2:
        for _, row in historik.iloc[::-1].iterrows():
            ...
            rating = hent_vaerdi_robust(row, 'Rating_Avg')
            with st.expander(f"Rapport fra {dato_str} (Rating: {rating})"):
                vis_metrikker(row)

    with tab3:
        fig_line = go.Figure()
        fig_line.add_trace(go.Scatter(x=historik['DATO_DT'], y=historik['Rating_Avg'], mode='lines+markers', line_color='#df003b'))
...
def vis_side():
    ...
    # RENS KOLONNENAVNE FOR AT UNDGÅ KEYERROR
    df.columns = [str(c).strip() for c in df.columns]
    
    # Find 'Dato' kolonnen uanset casing
    dato_col = next((c for c in df.columns if c.lower() == 'dato'), None)
    if not dato_col:
        st.error("Fejl: Kunne ikke finde en kolonne med navnet 'Dato'.")
        return

    df['DATO_DT'] = pd.to_datetime(df[dato_col], errors='coerce')
    ...
    with c2:
        with st.popover("Filtre"):
            f_pos = st.multiselect("Positioner", options=sorted(df['Position'].dropna().unique().tolist()) if 'Position' in df.columns else [])
            f_rating = st.slider("Rating", 1.0, 6.0, (1.0, 6.0), step=0.1)

    # Find seneste rapport pr ID
    f_df = df.groupby('ID').tail(1).copy()
    
    # Robust søgning og filtrering
    if search:
    ...
    f_df = f_df[(f_df['Rating_Avg'].astype(float) >= f_rating[0]) & (f_df['Rating_Avg'].astype(float) <= f_rating[1])]
    ...
    page_df_display.columns = [c.upper() for c in page_df_display.columns]
    ...
    if len(event.selection.rows) > 0:
        ...
        # Map tilbage til de rigtige navne
        p_data = {
            'ID': page_df.iloc[valgt_idx]['ID'],
            'NAVN': page_df.iloc[valgt_idx]['Navn'],
            'KLUB': page_df.iloc[valgt_idx]['Klub'],
            'POSITION': page_df.iloc[valgt_idx]['Position'],
            'RATING_AVG': page_df.iloc[valgt_idx]['Rating_Avg']
        }
