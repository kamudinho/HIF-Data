...
def vis_side(df_shots, df_spillere, hold_map):
    ...
    if df_s.empty:
        st.info(f"Der er ikke registreret skud for det valgte hold (ID: {TEAM_WYID}).")
        return
    ...
    with col_stats:
        ...
        # Tabel over skud (Rettet indrykning herunder)
        ...
        # Metrics (Rettet indrykning så de bliver i col_stats)
        SHOTS = len(df_plot)
