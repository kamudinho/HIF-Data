...
def save_to_github(new_row_df):
    ...
    if r.status_code == 200:
        ...
        # Vi sikrer os at rækkefølgen af kolonnerne matcher præcis din liste
...
def vis_side(df_players, df_playerstats):
    ...
    # --- 1. BYG MASTER-LOOKUP FRA WYSCOUT & LOKAL DATA ---
    sources = []
    if df_playerstats is not None:
        temp_stats = df_playerstats.copy()
        temp_stats['NAVN'] = temp_stats['FIRSTNAME'] + " " + temp_stats['LASTNAME']
        sources.append(temp_stats[['NAVN', 'PLAYER_WYID', 'TEAMNAME', 'ROLECODE3']].rename(
            columns={'TEAMNAME': 'Klub', 'ROLECODE3': 'Position'}
        ))
    if df_players is not None:
        sources.append(df_players[['NAVN', 'PLAYER_WYID', 'HOLD', 'POS']].rename(
            columns={'HOLD': 'Klub', 'POS': 'Position'}
        ))

    lookup_df = pd.concat(sources, ignore_index=True).drop_duplicates(subset=['NAVN'])
    ...
    with c1:
        if kilde == "Vælg eksisterende":
            ...
            if valgt_navn:
                ...
                klub_val = str(match.get('Klub', ''))
                pos_val = str(match.get('Position', ''))
        else:
            navn_endelig = st.text_input("Navn på ny spiller")
            p_id_val = st.text_input("PLAYER_WYID", placeholder="Indtast WyID")
    ...
    if navn_endelig and kilde == "Vælg eksisterende":
        st.caption(f"Valgt PLAYER_WYID: {p_id_val}")
    ...
    with st.form("scout_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Gem rapport", use_container_width=True):
            if navn_endelig and p_id_val:
                ...
                # RÆKKEFØLGEN SKAL MATCHE DINE KOLONNER PRÆCIS:
                ny_df = pd.DataFrame([[
                    p_id_val, datetime.now().strftime("%Y-%m-%d"), navn_endelig, klub_input, pos_input, 
