...
def vis_side(df):
    ...
    try:
        ...
        # --- 1. STRIKT LIGA-FILTER (Gøres som det allerførste) ---
        # Vi tvinger df til KUN at indeholde NordicBet Liga (328)
        ...
        # 3. KLARGØR NAVNE (Nu kun fra liga 328)
        ...
        # 4. DYNAMISK HOLD-LISTE (Nu garanteret kun liga 328)
        ...
        # Vi definerer f_df her for at sikre at vi altid har en dataframe til visning
        if valgt_hold != "Alle hold":
            f_df = df[(df['HOME_TEAM'] == valgt_hold) | (df['AWAY_TEAM'] == valgt_hold)].copy()
            
            # Beregn stats på f_df
            ...
            f_df = df.copy()
            stats_html = f"<div style='padding-top:7px;'><span style='font-size:14px; color:#555;'>Viser alle {len(f_df)} liga-kampe</span></div>"
        ...
        # --- 6. TABELVISNING (Baseret på f_df) ---
