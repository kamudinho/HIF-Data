...
# Mappen med ligaer og deres ID'er
COMP_MAP = { 
    335: "Superliga", 
    328: "Betinia Ligaen", 
    329: "2. division", 
...
# Aldersgrænser (Max alder tilladt ved slutningen af året/sæsonen)
AGE_LIMITS = {
    1305: 19,
    1306: 17,
    1307: 15
...
def vis_side():
    ...
    # Hent CSV og fiks variabel-navnet (df_csv i stedet for df__csv)
    ...
    with col_left:
        ...
        if sel_id:
            ...
            with c2:
                alder_tekst = f"({alder} år)" if alder else ""
                st.write(f"**{p['NAVN']}** {alder_tekst}")
            ...
            with st.form("transfer_form", clear_on_submit=True):
                ...
                ny_klub = st.selectbox("Vælg ny klub (alle rækker)", hold_liste)
                ...
                if st.form_submit_button("SEND TRANSFER TIL DATABASEN"):
                    ...
                    if max_alder and alder and alder > max_alder:
                        st.error(f"⚠️ Spilleren er for gammel ({alder} år) til {COMP_MAP.get(ny_liga_id)} (Max {max_alder} år).")
                    else:
                        ny_række = {
                        ...
                        # Opdater lokal dataframe og send til GitHub
                        ...
                        if res_code in [200, 201]:
                            st.success(f"Gemt! Spilleren er flyttet.")
                            time.sleep(1)
                            st.rerun()
    ...
    with col_right:
        ...
        if sql_trup is not None:
            ...
            if valgt_hold:
                ...
                # Fuzzy match på klubnavn for visning fra CSV
                csv_filt = df_csv[
                ...
                final_trup = pd.concat([
                    sql_filt[['NAVN', 'POSITION', 'PLAYER_WYID']], 
                    csv_filt[['NAVN', 'POSITION', 'PLAYER_WYID']]
                ], ignore_index=True).sort_values(by='NAVN')
