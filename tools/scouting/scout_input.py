...
def render_rapport_indhold(report, keys, unique_suffix=""):
  ...
  with col_radar:
    ...
    st.plotly_chart(
        ...
        key=f"radar_{report.get('DATO')}_{unique_suffix}",
    )
...
def show_report_popup(valgt_navn, alle_rapporter, billed_map):
  spiller_historik = alle_rapporter[
      alle_rapporter["NAVN"] == valgt_navn
  ...
  img_url = billed_map.get(pid) or (
      f"https://cdn5.wyscout.com/photos/players/public/{pid}.png"
  )
...
def show_create_player_dialog():
  st.write(
      "Indtast oplysninger på spilleren, der ikke findes i systemet i forvejen."
  )
  with st.form("create_player_form"):
    ny_navn = st.text_input("Spillernavn *")

    # Klubløs / Kontrakt logik
    er_klublos = st.checkbox("Spilleren er klubløs")
    if er_klublos:
      ny_klub = "Klubløs"
      ny_kontrakt = "Klubløs"
    else:
      ny_klub = st.text_input("Klub *")
      kontrakt_metode = st.radio(
          "Angiv kontrakt via:", ["Kalender (Dato)", "Kontraktlængde (År)"], horizontal=True
      )
      if kontrakt_metode == "Kalender (Dato)":
        ny_kontrakt_dato = st.date_input(
            "Kontraktudløb", value=datetime.today().date()
        )
        ny_kontrakt = str(ny_kontrakt_dato)
      else:
        aar = st.selectbox("Kontraktlængde", [1, 2, 3, 4, 5])
        udloeb_dato = datetime.today().date().replace(
            year=datetime.today().year + aar
        )
        ny_kontrakt = str(udloeb_dato)

    ny_pos = st.selectbox(
        "Position",
        ["CB", "LB", "RB", "DMF", "CM", "AMF", "LW", "RW", "CF", "GK"],
    )

    # Fødselsdato via kalender
    ny_birth_date = st.date_input(
        "Fødselsdato",
    ...
    submitted = st.form_submit_button(
        "Send spilleren ind i databasen", use_container_width=True
    )
    if submitted:
      if not ny_navn or (not er_klublos and not ny_klub):
        st.error(
            "* Navn og klub skal udfyldes (medmindre spilleren er klubløs)!"
        )
      else:
        ...
        init_rapport = {
            "PLAYER_WYID": lokal_id,
            "DATO": datetime.now().strftime("%Y-%m-%d"),
            "NAVN": ny_navn,
            "KLUB": ny_klub,
            "POSITION": ny_pos,
            "BIRTHDATE": ny_birth_str,
            "RATING_AVG": 0.0,
            "STATUS": "Interessant",
            "POTENTIALE": "Middel",
            "STYRKER": "Oprettet manuelt",
            "UDVIKLING": "-",
            "VURDERING": "-",
            "BESLUTSOMHED": 3.0,
            "FART": 3.0,
            ...
            "TEKNIK": 3.0,
            "SPILINTELLIGENS": 3.0,
            "SCOUT": st.session_state.get("user", "HIF Scout"),
            "KONTRAKT": ny_kontrakt,
            "FORVENTNING": "Realistisk",
            "POS_PRIORITET": "B - Trupspiller",
            "POS": "1",
            "LON": "0",
            "SKYGGEHOLD": False,
            "KOMMENTAR": "Manuelt oprettet spiller",
            "ER_EMNE": False,
            "TRANSFER_VINDUE": "Sommer 27",
            "POS_343": 0.0,
            "POS_433": 0.0,
            "POS_352": 0.0,
        }
        ...
        df_old = (
            pd.read_csv(StringIO(content), low_memory=False)
            if content
            else pd.DataFrame(columns=COL_ORDER)
        )
        df_final = pd.concat(
            [df_old, pd.DataFrame([init_rapport])], ignore_index=True
        )[COL_ORDER]

        if (
            push_to_github(
                FILE_PATH,
                f"Oprettet manuel spiller: {ny_navn}",
...
def vis_side(dp):
  ...
  if not df_wyscout.empty and "IMAGEDATAURL" in df_wyscout.columns:
    billed_map = dict(
        zip(
            df_wyscout["PLAYER_WYID"].apply(rens_id),
            df_wyscout["IMAGEDATAURL"],
        )
    )
    ...
    for _, r in df.iterrows():
      ...
      navn = (
          ...
          if f or l
              ...
              get_safe_val(r, "PLAYER_NAME")
              or get_safe_val(r, "NAVN")
              or "Ukendt"
          )
      )
      klub = (
          get_safe_val(r, "TEAMNAME") or get_safe_val(r, "KLUB") or "Ukendt"
      )
      if p_id not in unique_players:
        unique_players[p_id] = {
            ...
            "data": {
                "n": navn,
                "id": p_id,
                "pos": get_safe_val(r, "POSITION"),
                "klub": klub,
                "birth": get_safe_val(r, "BIRTHDATE"),
            },
        }
  ...
  options_list = sorted(
      list(unique_players.keys()), key=lambda x: unique_players[x]["label"]
  )
  ...
  with r1c1:
    sel_id = st.selectbox(
        "Vælg spiller",
        [""] + options_list,
        format_func=lambda x: unique_players[x]["label"]
        if x
        else "Vælg spiller...",
    )
    if sel_id:
  ...
  if sel_id and not df_local.empty:
    ...
    if not m.empty:
      existing_report = m.sort_values("DATO", ascending=False).iloc[0]

  with r1c2:
    st.text_input(
        "Seneste rapport",
        value=existing_report["DATO"] if existing_report is not None else "-",
        disabled=True,
    )
  with r1c3:
    ...
    if existing_report is not None:
      if st.button("Åbn rapport", use_container_width=True):
        show_report_popup(data["n"], df_local, billed_map)
  with r1c4:
    st.markdown("<p style='margin-bottom: 28px;'></p>", unsafe_allow_html=True)
  ...
  scout_navn = r2c4.text_input(
      "SCOUT", value=st.session_state.get("user", "HIF Scout"), disabled=True
  )
  ...
  with st.form("rapport_form", clear_on_submit=True):
    with st.container(border=True):
      ...
      status = l2c1.selectbox(
          "Status", ["Interessant", "Hold øje", "Kig nærmere", "Køb", "Prioritet"]
      )
      ...
      forvent = l3c2.selectbox(
          "Forventning", ["Realistisk", "Kræver overtalelse", "Svær"]
      )
    ...
    if st.form_submit_button("Gem Rapport", use_container_width=True):
      if data["n"]:
        avg = round(
            sum([b_des, b_fart, b_agg, b_att, b_udh, b_led, b_tek, b_int]) / 8,
            2,
        )
        ny_rapport = {
            "PLAYER_WYID": data["id"],
            ...
            "UDVIKLING": udv,
            "VURDERING": vurder,
            "BESLUTSOMHED": float(b_des),
            "FART": float(b_fart),
            "AGGRESIVITET": float(b_agg),
            "ATTITUDE": float(b_att),
            "UDHOLDENHED": float(b_udh),
            "LEDEREGENSKABER": float(b_led),
            "TEKNIK": float(b_tek),
            "SPILINTELLIGENS": float(b_int),
            "SCOUT": scout_navn,
            "KONTRAKT": str(kontrakt),
            "FORVENTNING": forvent,
            "POS_PRIORITET": prio,
            "POS": pos_nr,
            "LON": f"{lon:,}".replace(",", "."),
        ...
        df_final = pd.concat(
            [df_old, pd.DataFrame([ny_rapport])], ignore_index=True
        )[COL_ORDER]
        if (
            push_to_github(
                FILE_PATH,
