...
def vis_side():
    ...
    with t3:
        # Skyggeliste viser nu bare hele listen til redigering uden dropdown filter
        ...
        if not df_sky.empty:
            ...
            st.data_editor(d_sky_ed, use_container_width=True, height=600, key="sky_ed_final")

    with t4:
        c_pitch, c_ctrl = st.columns([8.5, 1.5])
        
        with c_ctrl:
            # Her vises dropdown stadig til at styre banen
