...
def get_github_file(path):
    # Vi tilføjer tidsstempel for at undgå caching fra GitHubs side
...
def prepare_df(content, is_hif=False):
    ...
    # Rens for dubletter i kolonnenavne
    df = df.loc[:, ~df.columns.duplicated()]
    
    if 'NAVN' in df.columns: df = df.rename(columns={'NAVN': 'Navn'})
    df = df.dropna(subset=['Navn'])
    
    # TVING kolonnerne frem hvis de ikke findes
...
def vis_side():
    ...
    for tab, df_display, path, key in configs:
        with tab:
            if not df_display.empty:
                # Vi definerer de kolonner vi VIL se eksplicit
                cols = ['TRANSFER_VINDUE', 'POS', 'KLUB', 'KONTRAKT', 'SKYGGEHOLD']
                df_input = df_display.set_index('Navn')[cols]
                ...
                if not ed.equals(df_input):
                    ...
                    push_to_github(path, "Update", df_save.to_csv(index=False), sha_raw)
                    st.rerun()

    # --- TAB 3 (DIN SKYGGELISTE FRA SCREENSHOT) ---
    with t3:
                ...
                for navn, row in ed_s.iterrows():
                    for p in [SCOUT_DB_PATH, HIF_PATH]:
                        ...
                        if navn in df_tmp['Navn'].values:
                            ...
                            push_to_github(p, "Tactical/Window Update", df_tmp.to_csv(index=False), sha_raw)
                st.rerun()
    ...
    with t4:
        ...
        if not df_total.empty:
            valgt = st.selectbox("Vælg trup-vindue:", VINDUE_OPTIONS)
            df_filtered = df_total[df_total['TRANSFER_VINDUE'].isin(["Nu", valgt])]
            
            # (Pitch tegning logik herunder er den samme som før...)
            ...
            with c_p:
                ...
                for pid, (x, y, lbl) in m.items():
                    ...
                    for i, (_, p) in enumerate(players.iterrows()):
                        ...
                        ax.text(x, y+(i*3.5), txt, size=7, ha='center', weight='bold', bbox=dict(facecolor=bg, edgecolor=edge, alpha=0.9, boxstyle='square,pad=0.1', linewidth=1.5 if p['TRANSFER_VINDUE'] != "Nu" else 1))
                st.pyplot(fig)
