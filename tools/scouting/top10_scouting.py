...
def vis_side(advanced_stats_df, position_base_df=None):
    ...
    if position_base_df is not None and not position_base_df.empty:
        ...
        id_adv = next((c for c in df.columns if 'PLAYER_WYID' in c or 'WYID' in c), None)
        id_pos = next((c for c in pos_base.columns if 'PLAYER_WYID' in c or 'WYID' in c), None)
        ...
        if id_adv and id_pos:
            ...
            try:
                ...
                if beregned_pos is not None and not beregned_pos.empty:
                    ...
                    b_id = next((c for c in beregned_pos.columns if 'PLAYER_WYID' in c or 'TEMP_ID' in c), None)
                    b_grp = next((c for c in beregned_pos.columns if 'PRIMAER_POSITIONSGRUPPE' in c or 'POS_GROUP' in c), None)
                    if b_id and b_grp:
    ...
    # Sorter grupperne efter den definerede rækkefølge i positional_helper
    ...
    # Find den kolonne der indeholder 'COMPETITION_WYID' (selv hvis den sidder sammen med andre ord)
    komp_col = next((c for c in df.columns if 'COMPETITION_WYID' in c), None)
    if not komp_col:
        st.error("Kunne ikke finde turneringskolonnen (COMPETITION_WYID) i data.")
        return
    ...
    for idx, (komp_id, liga_navn) in enumerate(liga_mapping.items()):
        with kolonner[idx]:
            ...
            # Filtrer sikkert på om værdien indeholder competition ID'et
            ...
            mins_col = next((c for c in df.columns if 'MINUTESONFIELD' in c or 'MINUTES' in c), None)
            ...
            for metrik_def in gruppe_definitioner:
                ...
                if beregn_type == "p90":
                    ...
                    match_col = next((c for c in liga_df.columns if kolonne in c), None)
                    if match_col:
                    ...
                    s_col = next((c for c in liga_df.columns if succes_kol in c), None)
                    t_col = next((c for c in liga_df.columns if total_kol in c), None)
                    if s_col and t_col:
            ...
            navn_col = next((c for c in df.columns if 'PLAYER_NAME' in c or 'SHORTNAME' in c or 'NAVN' in c), None)
            hold_col = next((c for c in df.columns if 'TEAMNAME' in c or 'KLUB' in c), None)
