...
def vis_side(dp):
    # CSS Optimering: Flexbox sikrer at ikoner og tekst står på linje
    st.markdown("""
        <style>
    ...
    with tab1:
        if df_skud.empty:
            ...
            col_viz, col_ctrl = st.columns([2.5, 1]) # Ændret ratio for at gøre banen smallere
            with col_ctrl:
            ...
            with col_viz:
                # Mindre figsize (6, 8) gør banen mindre dominerende
                ...
                pitch.scatter(df_vis['EVENT_X'], df_vis['EVENT_Y'], s=100, c=c_map, edgecolors=HIF_RED, linewidth=1.2, ax=ax)
                st.pyplot(fig)
    ...
    with tab2:
        if df_skud.empty:
            ...
            col_dz_viz, col_dz_ctrl = st.columns([2.5, 1])
            ...
            with col_dz_viz:
                ...
                fig_dz, ax_dz = pitch_dz.draw(figsize=(6, 7))
                ...
                pitch_dz.scatter(non_dz['EVENT_X'], non_dz['EVENT_Y'], s=50, c='white', edgecolors='#dddddd', alpha=0.2, ax=ax_dz)
                c_dz = (dz_hits['EVENT_TYPEID'] == 16).map({True: HIF_RED, False: 'white'})
                pitch_dz.scatter(dz_hits['EVENT_X'], dz_hits['EVENT_Y'], s=150, c=c_dz, edgecolors=HIF_RED, linewidth=1.5, ax=ax_dz)
                st.pyplot(fig_dz)
            ...
            for spiller in spiller_liste_dz:
                ...
                stats_list.append({"Navn": efternavn, "Skud i DZ": len(s_dz), "Mål i DZ": len(s_dz[s_dz['EVENT_TYPEID'] == 16]), "DZ %": (len(s_dz)/len(s_data)*100) if len(s_data) > 0 else 0})
            
            df_table_data = pd.DataFrame(stats_list).sort_values("Skud i DZ", ascending=False)
            fig_tab, ax_tab = plt.subplots(figsize=(10, 2.0)) # Mindre tabel
            ax_tab.axis('off')
            the_table = ax_tab.table(cellText=[[int(v) for v in df_table_data['Skud i DZ'].values], [int(v) for v in df_table_data['Mål i DZ'].values], [f"{v:.1f}%" for v in df_table_data['DZ %'].values]], rowLabels=["Skud i DZ", "Mål i DZ", "DZ %"], colLabels=df_table_data['Navn'].values, loc='top', cellLoc='center')
            ...
            fig_tab.subplots_adjust(top=0.8, bottom=0)
            st.pyplot(fig_tab)
    ...
    with tab3:
        if df_assists.empty:
            ...
            with col_viz_a:
                ...
                fig_a, ax_a = pitch_a.draw(figsize=(6, 8))
                if not df_a_vis.empty:
                    ...
                    pitch_a.scatter(df_a_vis['PASS_START_X'], df_a_vis['PASS_START_Y'], 
                                    s=100, color=HIF_GOLD, edgecolors='black', linewidth=1, ax=ax_a, zorder=2)
                st.pyplot(fig_a)
