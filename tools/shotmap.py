...
def vis_side(dp):
    st.markdown("""
        <style>
            ...
            /* Fjerner ekstra padding i Streamlit tabs */
    ...
    with tab2:
        if df_skud.empty:
            ...
            with col_dz_viz:
                ...
                # Fjern hvid plads i bunden af figuren
                fig_dz.subplots_adjust(bottom=0)
                st.pyplot(fig_dz)

            # TABEL: Rykket helt op (ingen divider)
            stats_list = []
            ...
            # Formindsket figsize og fjernet margins
            fig_tab, ax_tab = plt.subplots(figsize=(12, 2.5)) 
            ax_tab.axis('off')
            ...
            the_table = ax_tab.table(
                ...
                colLabels=df_table_data['Spiller'].values,
                loc='top', # Rykket til toppen af figuren
                cellLoc='center'
            )
            ...
            # Her fjernes alt hvidt luft omkring tabellen
            fig_tab.subplots_adjust(top=0.9, bottom=0, left=0.1, right=0.9)
            st.pyplot(fig_tab)
