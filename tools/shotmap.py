...
def vis_side(dp):
    ...
    with tabs[2]:
        ...
        with c2:
            ...
            st.markdown(f'<div class="stat-box" style="border-left-color:{HIF_RED}"><div class="stat-label"><span class="legend-dot" style="background:{HIF_RED};"></span>Antal mål, samlet</div><div class="stat-value">{m_cnt}</div></div>', unsafe_allow_html=True)
    ...
    def zone_plot_enhanced(data, is_m):
        ...
        for zone, b in ZONE_BOUNDARIES.items():
            ...
            zone_stats[zone] = {'cnt': cnt, 'pct': pct, 'top': top_p}
