...
def vis_side(dp):
    ...
    if df_raw.empty:
        st.warning("Data Provider returnerede en tom tabel. Tjek SQL-forbindelsen.")
        return
    ...
    with tab1:
        ...
        st.markdown(f"""
            <div class="stat-container">
                <div class="stat-box"><div class="stat-label">Skud</div><div class="stat-value">{n_skud}</div></div>
        ...
        if not df_skud.empty:
            ...
            pitch.scatter(df_skud[df_skud.EVENT_TYPEID == 16].EVENT_X, 
                         ...
                         s=200, c=HIF_RED, edgecolors='black', label='Mål', ax=ax)
            # Missede
            pitch.scatter(df_skud[df_skud.EVENT_TYPEID != 16].EVENT_X, 
                         df_skud[df_skud.EVENT_TYPEID != 16].EVENT_Y, 
                         s=150, c='white', edgecolors=HIF_RED, label='Andet skud', ax=ax)
    ...
    with tab2:
        ...
        if not df_chance.empty:
            ...
            ax_a.legend(loc='lower center', bbox_to_anchor=(0.5, 0.05), ncol=2)
        st.pyplot(fig_a)
