...
def generer_anbefaling(df_fjende):
    ...
    # Analyse af pasningssikkerhed på egen halvdel (Indikation på pressårbarhed)
    egen_halvdel_pas = df_fjende[(df_fjende['EVENT_TYPEID'] == 1) & (df_fjende['EVENT_X'] < 40)]
    if not egen_halvdel_pas.empty:
        pass_acc = (egen_halvdel_pas['OUTCOME'].sum() / len(egen_halvdel_pas)) * 100
        if pass_acc < 75:
            anbefalinger.append("ANBEFALING: Højt pres. Modstanderen har lav succesrate i opbygningsspillet.")

    # Analyse af felt-beskyttelse
    felt_aktioner = len(df_fjende[(df_fjende['EVENT_X'] < 20) & (df_fjende['EVENT_Y'].between(20, 80))])
    if felt_aktioner > 15:
        anbefalinger.append("ANBEFALING: Direkte spil i feltet. Modstanderen tillader mange aktioner i egen boks.")

    # Analyse af duelstyrke
    dueller = df_fjende[df_fjende['EVENT_TYPEID'].isin([7, 44])]
    if not dueller.empty:
        duel_acc = (dueller['OUTCOME'].sum() / len(dueller)) * 100
        if duel_acc < 45:
            anbefalinger.append("ANBEFALING: Fysisk dominans. Modstanderen taber statistisk set flertallet af deres dueller.")

    if not anbefalinger:
        anbefalinger.append("ANBEFALING: Disciplineret struktur. Fokus på minimering af egne fejl.")
