...
def draw_match_row(date, h_name, h_uuid, score, a_name, a_uuid, res_char):
    ...
    with cols[0]: st.markdown(f"<span style='font-size:11px; color:#666;'>{date}</span>", unsafe_allow_html=True)
    with cols[1]: st.markdown(f"<div style='text-align:right; font-size:13px; font-weight:600;'>{h_name[:12]}</div>", unsafe_allow_html=True)
    with cols[2]:
        logo = next((i['logo'] for n, i in TEAMS.items() if i.get('opta_uuid') == h_uuid), None)
        if logo: st.image(logo, width=18)
    with cols[3]: st.markdown(f"<div style='background:#f0f2f6; border-radius:4px; text-align:center; font-weight:800; font-size:12px;'>{score}</div>", unsafe_allow_html=True)
    with cols[4]:
        logo = next((i['logo'] for n, i in TEAMS.items() if i.get('opta_uuid') == a_uuid), None)
        if logo: st.image(logo, width=18)
    with cols[5]: st.markdown(f"<div style='text-align:left; font-size:13px; font-weight:600;'>{a_name[:12]}</div>", unsafe_allow_html=True)
    with cols[6]: st.markdown(f"<div style='background:{bg_color}; color:white; border-radius:4px; text-align:center; font-weight:bold; width:22px; font-size:11px;'>{res_char}</div>", unsafe_allow_html=True)
...
def plot_custom_pitch(df, event_ids, title, zone='full', cmap='Reds', logo=None):
    ...
    if zone == 'up': 
        ax.set_ylim(0, 55)
        logo_y, text_y = 0.03, 0.05
    else: 
        ax.set_ylim(45, 100)
        logo_y, text_y = 0.90, 0.97
    if logo:
        ax_l = ax.inset_axes([0.04, logo_y, 0.08, 0.08], transform=ax.transAxes)
        ax_l.imshow(logo); ax_l.axis('off')
    ax.text(0.94, text_y, title, transform=ax.transAxes, fontsize=8, fontweight='bold', ha='right')
...
def vis_side(dp=None):
    ...
    m_ids_all = df_all_matches['MATCH_OPTAUUID'].tolist()
    m_ids_all_str = format_id_list(m_ids_all)
    df_res_10 = df_all_matches.head(10)
    m_ids_10_str = format_id_list(df_res_10['MATCH_OPTAUUID'].tolist())

    # 2. Hent Alle Events (Hele sæsonen) for at undgå 'undefined' fejl senere
    df_events_all = conn.query(f"SELECT *, EVENT_OUTCOME as OUTCOME FROM {DB}.OPTA_EVENTS WHERE EVENT_CONTESTANT_OPTAUUID = '{valgt_uuid}' AND MATCH_OPTAUUID IN {m_ids_all_str}")
    df_events_10 = df_events_all[df_events_all['MATCH_OPTAUUID'].isin(df_res_10['MATCH_OPTAUUID'].tolist())]

    # 3. Målsekvenser (Hele sæsonen)
    sql_seq = f"""
    WITH Goals AS (SELECT MATCH_OPTAUUID, EVENT_TIMESTAMP as G_TIME, EVENT_TIMEMIN as G_MIN FROM {DB}.OPTA_EVENTS WHERE EVENT_TYPEID = 16 AND MATCH_OPTAUUID IN {m_ids_all_str} AND EVENT_CONTESTANT_OPTAUUID = '{valgt_uuid}') 
    SELECT e.*, m.CONTESTANTHOME_NAME, m.CONTESTANTAWAY_NAME, m.CONTESTANTHOME_OPTAUUID, m.CONTESTANTAWAY_OPTAUUID, m.TOTAL_HOME_SCORE, m.TOTAL_AWAY_SCORE, m.MATCH_LOCALDATE, g.G_TIME as GOAL_TS, g.G_MIN 
    FROM {DB}.OPTA_EVENTS e JOIN {DB}.OPTA_MATCHINFO m ON e.MATCH_OPTAUUID = m.MATCH_OPTAUUID 
    INNER JOIN Goals g ON e.MATCH_OPTAUUID = g.MATCH_OPTAUUID AND e.EVENT_TIMESTAMP >= DATEADD(second, -15, g.G_TIME) AND e.EVENT_TIMESTAMP <= g.G_TIME 
    ...
    with t1:
        ...
        w, d = (df_res_10['RES']=="W").sum(), (df_res_10['RES']=="D").sum()
        m_cols = st.columns(5)
        m_cols[0].metric("Point", (w*3)+d); m_cols[1].metric("Vundne", w); m_cols[2].metric("Uafgjort", d); m_cols[3].metric("Tabte", (df_res_10['RES']=="L").sum())
        
        c1, c_sp, c2 = st.columns([1.3, 0.1, 2.0])
        with c1:
        ...
        with c2:
            ...
            st.info("Grafer her (baseret på df_events_10)")
    ...
    with t4:
        if df_goals_all is not None and not df_goals_all.empty:
            gl = df_goals_all.drop_duplicates(['MATCH_OPTAUUID', 'GOAL_TS']).sort_values('GOAL_TS', ascending=False)
            opt = {f"{r['MATCH_OPTAUUID']}_{r['GOAL_TS']}": f"{pd.to_datetime(r['MATCH_LOCALDATE']).strftime('%d/%m')} vs {r['CONTESTANTAWAY_NAME'] if r['CONTESTANTHOME_OPTAUUID']==valgt_uuid else r['CONTESTANTHOME_NAME']} ({int(r['G_MIN'])}. min)" for _, r in gl.iterrows()}
            sk = st.selectbox("Vælg mål", list(opt.keys()), format_func=lambda x: opt[x])
            
            tge = df_goals_all[(df_goals_all['MATCH_OPTAUUID'] == sk.split('_')[0]) & (df_goals_all['GOAL_TS'].astype(str) == sk.split('_')[1])].sort_values('EVENT_TIMESTAMP')
            p_c, l_c = st.columns([2.5, 1])
            p = Pitch(pitch_type='opta'); f, ax = p.draw()
            # Logo vs Logo + Score i bunden
            ...
            for i in range(len(tge)-1):
                p.arrows(tge.iloc[i]['EVENT_X'], tge.iloc[i]['EVENT_Y'], tge.iloc[i+1]['EVENT_X'], tge.iloc[i+1]['EVENT_Y'], ax=ax, width=1, color='black', alpha=0.3)
            p_c.pyplot(f)
    ...
    with t5:
        if not df_events_all.empty:
            ...
            df_events_all['is_pass'] = (df_events_all['EVENT_TYPEID'] == 1).astype(int)
            df_events_all['is_regain'] = df_events_all['EVENT_TYPEID'].isin([12, 127, 49]).astype(int)
            df_events_all['is_shot'] = df_events_all['EVENT_TYPEID'].isin([13,14,15,16]).astype(int)
            df_events_all['is_tib'] = ((df_events_all['EVENT_X'] > 83) & (df_events_all['EVENT_Y'] > 21.1) & (df_events_all['EVENT_Y'] < 78.9)).astype(int)
            stats = df_events_all.groupby('PLAYER_NAME').agg(Pasninger=('is_pass','sum'), Erobringer=('is_regain','sum'), Skud=('is_shot','sum'), Touches_in_Box=('is_tib','sum'))
