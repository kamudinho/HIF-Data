...
def vis_side(dp=None):
    ...
    with st.spinner("Rydder op i sekvenserne..."):
        df_matches = conn.query(f"SELECT * FROM {DB}.OPTA_MATCHINFO WHERE TOURNAMENTCALENDAR_OPTAUUID = '{LIGA_UUID}'")
        
        # SQL der bruger POSSESSIONID for at undgå "støj"
        sql_clean = f"""
        WITH GoalEvents AS (
            SELECT 
                MATCH_OPTAUUID, EVENT_TIMESTAMP, EVENT_CONTESTANT_OPTAUUID, EVENT_TIMEMIN, EVENT_TYPEID, POSSESSIONID,
        ...
        JOIN GoalList gl ON e.MATCH_OPTAUUID = gl.MATCH_OPTAUUID 
            AND e.POSSESSIONID = gl.POSSESSIONID -- KUN events fra samme angreb!
    ...
    with t2:
        ...
        if not team_seq.empty:
            ...
            goal_options = {row['GOAL_TIME']: {
                'label': f"Mål vs. {row['CONTESTANTAWAY_NAME'] if row['CONTESTANTHOME_OPTAUUID']==valgt_uuid else row['CONTESTANTHOME_NAME']} ({row['EVENT_TIMEMIN']}. min)",
            ...
            # Filtrering: Vis kun de sidste 5-7 aktioner i det angreb for at undgå rod
            ...
            with col_b:
                ...
                for i in range(len(this_goal)):
                    ...
                    if i < len(this_goal) - 1:
                        ...
                        # Tegn kun pilen hvis det er en reel sekvens (ikke et kæmpe hop)
