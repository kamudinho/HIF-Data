...
def draw_match_info_box(ax, scoring_team_logo, opp_team_logo, date_str, score_str, min_str):
    ...
    if scoring_team_logo:
        ax_l1 = ax.inset_axes([0.02, 0.08, 0.08, 0.08], transform=ax.transAxes)
        ax_l1.imshow(scoring_team_logo)
        ax_l1.axis('off')
    
    # "vs." tekst
    ax.text(0.05, 0.105, "vs.", transform=ax.transAxes, fontsize=8, fontweight='bold', va='center')
    
    # Logo 2
    if opp_team_logo:
        ax_l2 = ax.inset_axes([0.05, 0.08, 0.05, 0.05], transform=ax.transAxes)
        ax_l2.imshow(opp_team_logo)
        ax_l2.axis('off')
    
    # Info-linje (Rykket op fra 0.04 til 0.05)
    full_info = f"{date_str}  |  Resultat: {score_str}  ({min_str}. min)"
    ax.text(0.02, 0.07, full_info, transform=ax.transAxes, fontsize=8, color='#444444', va='top', fontweight='medium')
