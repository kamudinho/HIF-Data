...
def vis_side(dp=None):
    ...
    with st.spinner("Henter og analyserer Opta data..."):
        ...
        # 3.2 SQL til mål-sekvenser (OPTIMERET)
        sql_seq = f"""
        ...
        GoalMetadata AS (
            SELECT 
                ...
                SUM(CASE WHEN g.EVENT_TYPEID = 16 THEN 1 ELSE 0 END) OVER (PARTITION BY g.MATCH_OPTAUUID, g.EVENT_CONTESTANT_OPTAUUID ORDER BY g.EVENT_TIMESTAMP) as TEAM_SCORE
            FROM {DB}.OPTA_EVENTS g
            JOIN {DB}.OPTA_MATCHINFO m ON g.MATCH_OPTAUUID = m.MATCH_OPTAUUID
            WHERE g.EVENT_TYPEID = 16 AND g.TOURNAMENTCALENDAR_OPTAUUID = '{LIGA_UUID}'
        )
        SELECT e.*, gm.SCORING_TEAM as GOAL_TEAM_ID, gm.GOAL_TIME, gm.EVENT_TIMEMIN as GOAL_MIN
        ...
        AND e.EVENT_TIMESTAMP <= COALESCE(gm.GOAL_TIME, e.EVENT_TIMESTAMP)
        """
        df_sequences = conn.query(sql_seq)

        # 3.3 SQL til Spiller-stats (RETTET: Separerer Assists korrekt via QID 213)
        sql_stats = f"""
    ...
    with t2:
        ...
        if not team_seq.empty:
            ...
            team_seq = team_seq.merge(df_matches[['MATCH_OPTAUUID', 'MATCH_LOCALDATE', 'CONTESTANTHOME_NAME', 'CONTESTANTAWAY_NAME', 'CONTESTANTHOME_OPTAUUID', 'CONTESTANTAWAY_OPTAUUID', 'TOTAL_HOME_SCORE', 'TOTAL_AWAY_SCORE']], on='MATCH_OPTAUUID', how='left')
            
            goal_list = team_seq[team_seq['EVENT_TYPEID'] == 16].drop_duplicates('GOAL_TIME').sort_values(by=['MATCH_LOCALDATE', 'GOAL_MIN'], ascending=[False, True])
            
            goal_options = {row['GOAL_TIME']: {
                'label': f"{row['MATCH_LOCALDATE'].strftime('%d/%m/%Y')}: Mål vs. {row['CONTESTANTAWAY_NAME'] if row['CONTESTANTHOME_OPTAUUID']==valgt_uuid else row['CONTESTANTHOME_NAME']} ({row['GOAL_MIN']}. min)",
    ...
    with t3:
        ...
        df_team_stats = df_team_stats.rename(columns={
            'PLAYER': 'Spiller', 'GOALS': 'Mål', 'ASSISTS': 'Assists', 
            'PASSES_IN_GOAL': 'Pasninger (før mål)', 'TOTAL_ACTIONS': 'Total involveringer'
        })
