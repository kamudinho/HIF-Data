...
def draw_match_row(date, h_name, h_uuid, score, a_name, a_uuid, res_char):
    ...
    with col2: st.markdown(f"<p style='font-size:13px; font-weight:600; margin:8px 0; text-align:right;'>{h_name[:12]}</p>", unsafe_allow_html=True)
    with col3:
    ...
    with col6: st.markdown(f"<p style='font-size:13px; font-weight:600; margin:8px 0;'>{a_name[:12]}</p>", unsafe_allow_html=True)
