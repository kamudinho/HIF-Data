...
def plot_custom_pitch(df, event_ids, title, zone='full', cmap='Reds', logo=None):
    # FILTRERING OG FIGUR-GENERERING
    ...
    return fig # VIGTIGT: Returner figuren så den kan tegnes
...
def vis_side(dp=None):
    ...
    # Målsekvenser (Hele sæsonen) - DEFINERES HER SÅ DEN ER KLAR TIL T4
    sql_seq = f"""
    WITH Goals AS (
        SELECT MATCH_OPTAUUID, EVENT_TIMESTAMP as G_TIME, EVENT_TIMEMIN as G_MIN 
        FROM {DB}.OPTA_EVENTS 
        WHERE EVENT_TYPEID = 16 AND MATCH_OPTAUUID IN {m_ids_all_str} AND EVENT_CONTESTANT_OPTAUUID = '{valgt_uuid}'
    ) 
    SELECT e.*, m.CONTESTANTHOME_NAME, m.CONTESTANTAWAY_NAME, m.CONTESTANTHOME_OPTAUUID, m.CONTESTANTAWAY_OPTAUUID, 
           m.TOTAL_HOME_SCORE, m.TOTAL_AWAY_SCORE, m.MATCH_LOCALDATE, g.G_TIME, g.G_MIN 
    FROM {DB}.OPTA_EVENTS e 
    JOIN {DB}.OPTA_MATCHINFO m ON e.MATCH_OPTAUUID = m.MATCH_OPTAUUID 
    INNER JOIN Goals g ON e.MATCH_OPTAUUID = g.MATCH_OPTAUUID 
    AND e.EVENT_TIMESTAMP >= DATEADD(second, -15, g.G_TIME) AND e.EVENT_TIMESTAMP <= g.G_TIME 
    WHERE e.EVENT_CONTESTANT_OPTAUUID = '{valgt_uuid}'
    """
    df_goals_all = conn.query(sql_seq)
    
    # Event data til de andre faner
    ...
    with t1:
        # Layout som requested: Metrics øverst, så kolonne-opdeling
        ...
        c1, c2, c3 = st.columns([1.3, 0.1, 2.0])
        with c1:
    ...
    with t4:
        if df_goals_all is not None and not df_goals_all.empty:
            ...
            tge = df_goals_all[(df_goals_all['MATCH_OPTAUUID'] == sk.split('_')[0]) & (df_goals_all['G_TIME'].astype(str) == sk.split('_')[1])].sort_values('EVENT_TIMESTAMP')
    ...
    with t5:
        # Pr. 90 min logik (Alle kampe)
        df_events_all = conn.query(f"SELECT EVENT_TYPEID, PLAYER_NAME, EVENT_X, EVENT_Y FROM {DB}.OPTA_EVENTS WHERE EVENT_CONTESTANT_OPTAUUID = '{valgt_uuid}' AND MATCH_OPTAUUID IN {m_ids_all_str}")
