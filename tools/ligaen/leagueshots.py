...
def vis_side(dp):
    ...
    with tabs[0]:
        ...
        for player in df_skud['PLAYER_NAME'].unique():
            ...
            stats.append({
                "UUID": t_uuid, "Spiller": player, "S": s, "M": m, 
                "xG": round(d['EXPECTED_GOALS_VALUE'].sum(), 2) if 'EXPECTED_GOALS_VALUE' in d.columns else 0,
                "K%": round((m/s*100),1) if s > 0 else 0,
                "DZ-S": len(dz), "DZ-M": len(dz[dz['EVENT_TYPEID'] == 16]),
                "DZ%": int(len(dz)/s*100) if s > 0 else 0
            })
        ...
        for i, r in df_top.reset_index(drop=True).iterrows():
            ...
            html += f"<tr><td>{i+1}</td><td>{img}</td><td style='text-align:left;'><b>{r['Spiller']}</b></td><td>{r['S']}</td><td style='color:{HIF_RED}; font-weight:bold;'>{r['M']}</td><td>{r['xG']}</td><td>{r['K%']}%</td><td>{r['DZ-S']}</td><td>{r['DZ-M']}</td><td>{r['DZ%']}%</td></tr>"
        st.markdown(html + "</tbody></table>", unsafe_allow_html=True)

    # --- ZONE PLOT FUNKTION (RETTET) ---
    def zone_viz(is_m, key):
        ...
        with c2:
            ...
            st.dataframe(z_stats[z_stats['Zone'] != 'Zone 8'], hide_index=True)
        ...
        with c1:
            ...
            # HER ER FIXET: Vi bruger ZONE_BOUNDARIES
            for name, b in ZONE_BOUNDARIES.items():
                ...
                rect = patches.Rectangle((b["x_min"], y_min_draw), b["x_max"]-b["x_min"], b["y_max"]-y_min_draw, 
                                         facecolor=face, alpha=0.6, edgecolor='black', ls='--')
                ax.add_patch(rect)
                if cnt > 0:
                    ax.text(b["x_min"]+(b["x_max"]-b["x_min"])/2, y_min_draw+(b["y_max"]-y_min_draw)/2, 
    ...
    with tabs[2]:
        ...
        with c1:
            pitch = VerticalPitch(half=True, pitch_type='opta', line_color='#ccc')
            fig, ax = pitch.draw()
            ax.set_ylim(75, 105) # AFKORTET BANE
