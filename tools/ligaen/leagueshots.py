...
def vis_side(dp):
    ...
    if df_skud.empty:
        st.warning("Ingen liga-skuddata fundet i Snowflake.")
        return
    
    df_skud.columns = [c.upper() for c in df_skud.columns]
    col_team = 'EVENT_CONTESTANT_OPTAUUID'

    # Hjælpefunktion til logoer (identisk med din ligatabel-arkitektur)
    ...
    # --- 3. ZONE LOGIK ---
    # Danger Zone definition (centralt i feltet)
    ...
    # --- TAB 0: LIGAPROFILER (Hovedtabellen med Logoer og Bars) ---
    with tabs[0]:
        stats_list = []
        ...
        for p in df_skud['PLAYER_NAME'].unique():
            ...
            if s > 0:
                stats_list.append({
                    ...
                    "DZ-Skud": int(dz_s),
                    "DZ-Mål": int(dz_m),
                    "DZ-Konv.%": float(round((dz_m/dz_s*100), 1)) if dz_s > 0 else 0.0,
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Konv.%": st.column_config.NumberColumn("K%", format="%.1f%%"),
                "DZ-Skud": st.column_config.NumberColumn("DZ-S", format="%d"),
                "DZ-Mål": st.column_config.NumberColumn("DZ-M", format="%d"),
                "DZ-Konv.%": st.column_config.NumberColumn("DZ-K%", format="%.1f%%"),
    ...
    with tabs[2]:
        ...
        dz_stats = df_final[['Logo', 'Spiller', 'Klub', 'DZ-Skud', 'DZ-Mål', 'DZ-Konv.%']].sort_values('DZ-Skud', ascending=False)
        st.dataframe(dz_stats, use_container_width=True, hide_index=True, column_config={"Logo": st.column_config.ImageColumn("")})
