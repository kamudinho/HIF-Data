...
def vis_side(dp):
    ...
    with tabs[1]:
        ...
        with c2:
            ...
            st.markdown(f'<div class="stat-box" style="border-left-color:{t_color}"><div style="font-size:0.8rem">Skud ({t_sel})</div><div style="font-size:1.5rem;font-weight:800">{len(d_v)}</div></div>', unsafe_allow_html=True)
        with c1:
    ...
    def zone_tab(is_goal, k):
        ...
        with c1:
            ...
            for name, b in ZONE_BOUNDARIES.items():
                ...
                ax.add_patch(patches.Rectangle((b["x_min"], max(b["y_min"], 55)), b["x_max"]-b["x_min"], b["y_max"]-max(b["y_min"], 55), facecolor=t_colorz, alpha=alpha, edgecolor='black', ls='--'))
                if cnt > 0:
