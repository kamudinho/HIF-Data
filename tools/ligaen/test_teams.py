...
def draw_h2h_chart(team1, team2, metrics, labels, df_wy, chart_key, df_liga):
    ...
    for i, m in enumerate(metrics):
        ...
        fig.add_trace(go.Bar(x=[0, 1], y=[v1, v2], marker_color=["#df003b", "#0056a3"], width=0.5, xaxis=f"x{suffix}", yaxis=f"y{suffix}"))
        fig.add_annotation(dict(x=0.5, y=-0.1, xref=f"x{suffix} domain", yref=f"y{suffix} domain", text=f"<b>{labels[i]}</b>", showarrow=False))
...
def vis_side():
    ...
    def render_tabel(df):
        d = df.copy()
        d.insert(1, ' ', [get_logo_html(u) for u in d['UUID']])
        d['FORM'] = d['FORM'].apply(style_form)
