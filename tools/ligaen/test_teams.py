...
def load_liga_data():
    ...
    return conn.query(f"SELECT * FROM {db}.OPTA_MATCHINFO WHERE TOURNAMENTCALENDAR_OPTAUUID = 'dyjr458hcmrcy87fsabfsy87o'")
...
def beregn_tabel(df_matches):
    ...
    for _, row in df_matches.iterrows():
        ...
        if str(row['MATCH_STATUS']).lower() in ['played', 'full-time', 'finished']:
            ...
            for u, g_f, g_a in [(h_uuid, h_g, a_g), (a_uuid, a_g, h_g)]:
                stats[u]['K'] += 1; stats[u]['M+'] += g_f; stats[u]['M-'] += g_a
                if g_f > g_a: stats[u]['P'] += 3; stats[u]['V'] += 1; stats[u]['FORM'] += 'V'
                elif g_f == g_a: stats[u]['P'] += 1; stats[u]['U'] += 1; stats[u]['FORM'] += 'U'
...
def vis_side():
    ...
    # Opdel i Grundspil (første 132 kampe) og Slutspil (alle kampe)
    ...
    def render_tabel(df_in, filter_list=None):
        df = df_in.copy()
        if filter_list: df = df[df['UUID'].isin(filter_list)]
