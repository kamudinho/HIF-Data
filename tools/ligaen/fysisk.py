...
def vis_side(conn, name_map=None):
    # --- 0. CSS ---
    st.markdown("<style>.stAppViewBlockContainer { padding-top: 0px !important; } div.block-container { padding-top: 1rem !important; max-width: 98% !important; }</style>", unsafe_allow_html=True)
    ...
    # --- 2. HJÆLPEFUNKTIONER TIL OMREGNING ---
    def clean_mins(v):
        """Omregner både '90:00' og rå sekunder til decimal-minutter"""
        ...
        try:
            # Hvis det er rå sekunder
            val = float(s)
            return val / 60 if val > 500 else val # Logik: hvis tallet er højt, er det sekunder
        except: return 0.0
    ...
    # Check for kolonnenavne (Snowflake uppercase fix)
    df_phys.columns = [c.upper() for c in df_phys.columns]
    t_col = "TEAM_NAME" if "TEAM_NAME" in df_phys.columns else "TEAMNAME"
    if t_col not in df_phys.columns: t_col = "TEAM" # Sidste udvej
    ...
    with t1:
        if not df_phys.empty:
            df_phys['MINS_DEC'] = df_phys['MINUTES'].apply(clean_mins)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "MINS_DEC": st.column_config.NumberColumn("Minutter", format="%d min")
                },
                use_container_width=True, hide_index=True, height=700
            )

    with t3:
        # Fikset SQL til Liga-overblik med korrekte kolonnenavne
        df_league = conn.query(f"""
            SELECT 
                PLAYER_NAME, 
                "{t_col}" as KLUB,
            ...
            GROUP BY PLAYER_NAME, "{t_col}"
            HAVING TOTAL_MINS > 45
        """)
        ...
        for key, (label, fmt, col, title) in {
            ...
            with col:
                st.write(f"**{title}**")
                st.dataframe(df_league.nlargest(5, key)[['PLAYER_NAME', 'KLUB', key]], hide_index=True, use_container_width=True)
    ...
    with t4:
        ...
        if not df_meta.empty:
            ...
            if not df_m.empty:
                ...
                df_m['DISPLAY_NAME'] = df_m.apply(lambda r: p_map.get(str(r.get('OPTAID', r.get('optaId'))), r['PLAYER_NAME']), axis=1)
                
                st.dataframe(
                    df_m[['DISPLAY_NAME', t_col, 'MINUTES', 'KM', 'HI_RUN', 'TOP_SPEED']].sort_values('KM', ascending=False),
                    column_config={"KM": st.column_config.NumberColumn("KM", format="%.2f km")},
