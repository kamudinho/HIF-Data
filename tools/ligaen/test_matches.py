...
def vis_side(dp=None):
    ...
    SELECT 
        ...
        -- Her bruger vi CASE WHEN til at tælle "Touches in Box" korrekt
        COUNT(CASE 
            WHEN EVENT_X >= 81.0 
            AND EVENT_Y BETWEEN 20.0 AND 80.0 
            AND EVENT_TYPEID IN (1, 3, 4, 7, 13, 14, 15, 16, 17, 19, 24, 30) 
            THEN 1 END) AS TOUCHES_IN_BOX,
        ...
        for _, row in played_p.sort_values('MATCH_DATE_FULL', ascending=False).iterrows():
            ...
            with st.container(border=True):
                ...
                stats_conf = [
                    ...
                    ("HOME_PASSES", "AWAY_PASSES", "PASSES", "Afleveringer: Samlet", 0, ""),
