...
def vis_side(dp=None):
    ...
    with row2[0]:
        ...
        with c_side:
            valgt_side = st.selectbox("Side", ["Samlet", "Hjemme", "Ude"], label_visibility="collapsed")
    ...
    # Summary beregning med NaN-håndtering
    summary = {"K": len(played_p), "S": 0, "U": 0, "N": 0, "M+": 0, "M-": 0}
    for _, m in played_p.iterrows():
        is_h = m['CONTESTANTHOME_OPTAUUID'] == valgt_uuid
        # Her sikrer vi mod NaN ved at bruge to_numeric og fillna(0)
    ...
    with tab1:
        if played_p.empty:
            ...
            for t_uuid in all_played['CONTESTANTHOME_OPTAUUID'].unique():
                t_m = all_played[(all_played['CONTESTANTHOME_OPTAUUID'] == t_uuid) | (all_played['CONTESTANTAWAY_OPTAUUID'] == t_uuid)]
                avgs = {}
                for k in stat_keys:
                    v_h = pd.to_numeric(t_m[f"HOME_{k}"].where(t_m['CONTESTANTHOME_OPTAUUID'] == t_uuid), errors='coerce')
                    v_a = pd.to_numeric(t_m[f"AWAY_{k}"].where(t_m['CONTESTANTAWAY_OPTAUUID'] == t_uuid), errors='coerce')
                    combined = pd.concat([v_h.dropna(), v_a.dropna()])
                    avgs[k] = combined.mean() if not combined.empty else 0
                team_avgs[t_uuid] = avgs

            for _, row in played_p.sort_values('MATCH_DATE_FULL', ascending=False).iterrows():
                week_val = int(pd.to_numeric(row.get('WEEK'), errors='coerce') or 0)
                st.markdown(f"<div class='date-header'>RUNDE {week_val} — {row['MATCH_DATE_FULL'].strftime('%d. %b %Y').upper()}</div>", unsafe_allow_html=True)
                
                with st.container(border=True):
                    h_uuid, a_uuid = row['CONTESTANTHOME_OPTAUUID'], row['CONTESTANTAWAY_OPTAUUID']
                    h_n, a_n = opta_to_name.get(h_uuid, "Hjemme"), opta_to_name.get(a_uuid, "Ude")
                    h_score = int(pd.to_numeric(row.get('TOTAL_HOME_SCORE'), errors='coerce') or 0)
                    a_score = int(pd.to_numeric(row.get('TOTAL_AWAY_SCORE'), errors='coerce') or 0)
                    ...
                    c3.markdown(f"<div style='text-align:center;'><span class='score-pill'>{h_score} - {a_score}</span></div>", unsafe_allow_html=True)
                    ...
                    for hc, ac, s_key, lbl, dec, suf in stats_conf:
                        hv = float(pd.to_numeric(row.get(hc), errors='coerce') or 0)
                        av = float(pd.to_numeric(row.get(ac), errors='coerce') or 0)
                        h_avg = team_avgs.get(h_uuid, {}).get(s_key, 0)
                        a_avg = team_avgs.get(a_uuid, {}).get(s_key, 0)
                        hd, ad = hv - h_avg, av - a_avg
                        
                        h_diff = f" <span style='color:{'green' if hd>=0 else 'red'}; font-size:10px;'>({'+' if hd>=0 else ''}{hd:.{dec}f}{suf})</span>"
                        a_diff = f"<span style='color:{'green' if ad>=0 else 'red'}; font-size:10px;'>({'+' if ad>=0 else ''}{ad:.{dec}f}{suf})</span> "
                        h_pct = (hv / (hv + av) * 100) if (hv + av) > 0 else 50

                        st.markdown(f"""
                            <div style='display:flex; justify-content:space-between; font-size:11px; margin-top:8px;'>
                                <div style='text-align:left;'><b>{hv:.{dec}f}{suf}</b>{h_diff}</div>
                                <div style='color:#888;'>{lbl.upper()}</div>
                                <div style='text-align:right;'>{a_diff}<b>{av:.{dec}f}{suf}</b></div>
                            </div>
