    ...
    sql = f"""
        ...
        AdvancedEvents AS (
            SELECT 
                ...
                -- Skud i danger zone
                COUNT_IF(EVENT_TYPEID = 16 AND EVENT_X > 75 AND EVENT_Y BETWEEN 25 AND 75) AS DANGERZONE_SHOTS,
                
                -- Afleveringer i sidste tredjedel
                COUNT_IF(EVENT_TYPEID = 1 AND EVENT_OUTCOME = 1 AND EVENT_X > 66.6) AS PASSES_FINAL_THIRD,
                
                -- Fremadrettede afleveringer baseret på næste hændelse (LEAD_X)
                COUNT_IF(EVENT_TYPEID = 1 AND EVENT_OUTCOME = 1 AND LEAD_X > (EVENT_X + 10)) AS FORWARD_PASSES
            ...
            FROM (
                SELECT 
                    ...
                    EVENT_Y,
                    LEAD(EVENT_X) OVER (
                        PARTITION BY MATCH_OPTAUUID, CONTESTANT_OPTAUUID 
