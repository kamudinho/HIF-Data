...
def render_setpiece_analysis(df_team, sp_type, t_sel):
    ...
    with col_p:
        ...
        if sp_type == "Hjørnespark":
            ...
            ax.text(89.0, 56.0, f"{sp_type.upper()} ({side_sel.upper()})", fontsize=7, fontweight='bold', color='#555555', va='center')
            ...
            ax.text(89.0, 53.0, stats_line, fontsize=7, color='#666666', va='center')

            if hold_logo:
                # Holdnavn og logo placeret nederst i venstre side (X=3.0)
                ax.text(89.0, 58.0, t_sel.upper(), fontsize=8, fontweight='bold', color='#222222', va='center')
                ax_logo = ax.inset_axes([89.0, 58.0, 6.0, 6.0], transform=ax.transData)
