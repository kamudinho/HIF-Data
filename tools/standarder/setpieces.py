    ...
    with col_p:
        ...
        if sp_type == "Hjørnespark":
            ...
            if hold_logo:
                ...
                ax_logo = ax.inset_axes([97.0, 68.0, 6.0, 6.0], transform=ax.transData)
                ax_logo.imshow(hold_logo)
                ax_logo.axis('off')
                ax.text(97.0,64.0, t_sel.upper(), fontsize=8, fontweight='bold', color='#222222', va='center')
            
            # Tekst og statistik placeret i venstre side
            ax.text(97.0, 60.0, f"{sp_type.upper()} ({side_sel.upper()})", fontsize=7, fontweight='bold', color='#555555', va='center')
            ...
            ax.text(97.0, 52.0, stats_line, fontsize=7, color='#666666', va='center')
        else:
