...
def hent_navne_map() -> dict:
    """Henter og opbygger et sikkert opslagsspecifikt kort over spillernavne."""
    try:
        if hasattr(player_mapping, "PLAYER_MAPPING"):
            ...
            if isinstance(mapping_data, list):
                return {
                    ...
                    for r in mapping_data if (r.get("player_optauuid") or r.get("PLAYER_OPTAUUID")) and (r.get("navn") or r.get("NAVN"))
                }
            elif isinstance(mapping_data, dict):
                return {str(k).strip().lower().replace("t", ""): str(v) for k, v in mapping_data.items()}

        if hasattr(player_mapping, "df_spiller"):
            df = player_mapping.df_spiller
            if df is not None and not df.empty:
                uuid_col = next((c for c in df.columns if "uuid" in c.lower()), None)
                navn_col = next((c for c in df.columns if c.lower() in ["navn", "name", "visningsnavn"]), None)
                if uuid_col and navn_col:
                    return {
                        str(r[uuid_col]).strip().lower().replace("t", ""): str(r[navn_col])
                        for _, r in df.iterrows() if pd.notna(r[uuid_col]) and pd.notna(r[navn_col])
                    }
        return {}
    except Exception:
        return {}
...
def hent_holdliste(_conn) -> dict:
    ...
    # Sikr at Hvidovre altid er med som fallback
    if not team_map:
        team_map = {"Hvidovre": "t7490"}
...
def vis_side():
    ...
    df_spiller_df = df_events[
        df_events["player_optauuid"].astype(str) == str(valgt_player_uuid)
    ].copy()

    if df_spiller_df.empty:
        st.info(f"Ingen registrerede aktioner for {valgt_spiller_navn} i de hentede kampe.")
        return

    if "event_typeid" in df_spiller_df.columns:
        df_spiller_df["event_typeid"] = pd.to_numeric(df_spiller_df["event_typeid"], errors="coerce")

    df_spiller_df["Action_Label"] = df_spiller_df.apply(lambda r: map_event_to_category(r), axis=1)

    if "qual_list" not in df_spiller_df.columns:
        if "qualifiers" in df_spiller_df.columns:
            df_spiller_df["qual_list"] = df_spiller_df["qualifiers"].astype(str).str.split(",")
        else:
            df_spiller_df["qual_list"] = [[] for _ in range(len(df_spiller_df))]

    spiller_position = (
        df_spiller_df["position"].iloc[0]
        if "position" in df_spiller_df.columns and not df_spiller_df["position"].empty
        else "Ukendt"
    )
    ...
    if "outcome" not in df_spiller_df.columns:
        df_spiller_df["outcome"] = 0
    else:
        df_spiller_df["outcome"] = pd.to_numeric(df_spiller_df["outcome"], errors="coerce").fillna(0)

    df_filtreret = df_spiller_df[~df_spiller_df["Action_Label"].isin(["Pasning", "Indkast"])].copy()

    akt_stats = pd.DataFrame()
    if not df_filtreret.empty:
        akt_stats = (
            df_filtreret.groupby("Action_Label")
            .agg(Total=("outcome", "count"), Succes=("outcome", lambda x: (x == 1).sum()))
    ...
    with c_stats_side:
        ...
        total_akt = len(df_spiller_df)
        pas_df = df_spiller_df[df_spiller_df["event_typeid"] == 1]
        pas_count = len(pas_df)
        pas_acc = (pas_df["outcome"].eq(1).sum() / pas_count * 100) if pas_count > 0 else 0

        chancer_skabt = (
            akt_stats[akt_stats.index.str.contains("Key Pass|assist|Stor chance", case=False, na=False)]["Total"].sum()
            if not akt_stats.empty else 0
        )
        shots_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([13, 14, 15, 16])])
        cross_count = len(df_spiller_df[df_spiller_df["qual_list"].apply(lambda x: "2" in x if isinstance(x, (list, np.ndarray)) else False)])
        erob_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([49])])
        touch_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin(TOUCH_IDS)])
        drib_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([3])])
        regains_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([7, 8, 12, 49])])
        boldtab_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([50, 51])])
        def_count = len(df_spiller_df[df_spiller_df["event_typeid"].isin([7, 8])])

        fremad_count = len(
            df_spiller_df[
                (df_spiller_df["event_typeid"] == 1)
                & df_spiller_df["end_x"].notna()
                & (df_spiller_df["end_x"] > df_spiller_df["event_x"])
            ]
        ) if "end_x" in df_spiller_df.columns and "event_x" in df_spiller_df.columns else 0
    ...
    with c_pitch_side:
        ...
        with c_desc_col:
            st.markdown(
                f'<div style="text-align: right; margin-top: 6px; line-height: 1.2;"><span style="color: #666; font-size: 0.82rem;">{descriptions_visning.get(visning)}</span></div>',
                unsafe_allow_html=True,
            )
        ...
        df_plot = df_spiller_df.dropna(subset=["event_x", "event_y"])
        ...
        if not df_plot.empty:
            if visning == "Heatmap":
                ...
                for label, mask_fn, color, size, marker in AKTIONS_FARVER:
                    ...
                    if not d.empty:
                        noget_vist = True
                        ax.scatter(d.event_x, d.event_y, color=color, s=size, edgecolors="white", alpha=0.75, label=label, marker=marker, zorder=3)
                if noget_vist:
                    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.03), ncol=len(AKTIONS_FARVER), fontsize=8, frameon=False)
                ...
                if "end_x" not in df_plot.columns or "end_y" not in df_plot.columns:
                    ...
                    d = df_plot[(df_plot["event_typeid"] == 1) & (df_plot["end_x"] > 66.7) & (df_plot["end_x"] > df_plot["event_x"])].dropna(subset=["end_x", "end_y"])
