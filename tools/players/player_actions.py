...
AKTIONS_FARVER = [
    ...
    (
        "Afslutning",
        lambda df: df["event_typeid"].isin([13, 14, 15]),
        "#ff7f0e",
        60,
        "o",
    ),
    ...
    (
        "Defensiv",
        lambda df: df["event_typeid"].isin([5, 7, 8, 12, 49, 55]),
        "#9467bd",
        50,
        "D",
    ),
    ...
    try:
        ...
        if hasattr(player_mapping, "df_spiller"):
            ...
            if df is not None and not df.empty:
                uuid_col = next(
                    (c for c in df.columns if "uuid" in c.lower()), None
                )
                navn_col = next(
                    (
                        c
                        for c in df.columns
                        if c.lower() in ["navn", "name", "visningsnavn"]
                    ),
                    None,
                )
                if uuid_col and navn_col:
                    return {
                        ...
                        if pd.notna(r[uuid_col]) and pd.notna(r[navn_col])
                    }
...
def hent_holdliste(_conn) -> dict:
    ...
    if not df_teams_raw.empty:
        for _, r in df_teams_raw.iterrows():
            uuid_clean = (
                str(r["contestanthome_optauuid"]).lower().replace("t", "")
            )
...
def vis_side():
    ...
    spiller_options = {
        ...
        for _, r in df_hold_spillere[
            ["visningsnavn", "player_optauuid"]
    ...
    st.markdown(
        "<hr style='margin: 10px 0 20px 0; opacity: 0.3;'>",
        unsafe_allow_html=True,
    )

    with st.spinner("Henter spillerens aktioner..."):
        df_events, _, _ = hent_match_og_haendelsesdata(
    ...
    if df_spiller_df.empty:
        st.info(
            f"Ingen registrerede aktioner for {valgt_spiller_navn} i de hentede kampe."
        )
        return

    if "event_typeid" in df_spiller_df.columns:
        df_spiller_df["event_typeid"] = pd.to_numeric(
            df_spiller_df["event_typeid"], errors="coerce"
        )

    df_spiller_df["Action_Label"] = df_spiller_df.apply(
        lambda r: map_event_to_category(r), axis=1
    )

    if "qual_list" not in df_spiller_df.columns:
        if "qualifiers" in df_spiller_df.columns:
            df_spiller_df["qual_list"] = (
                df_spiller_df["qualifiers"].astype(str).str.split(",")
            )
        else:
            df_spiller_df["qual_list"] = [
                [] for _ in range(len(df_spiller_df))
            ]

    spiller_position = (
        df_spiller_df["position"].iloc[0]
        if "position" in df_spiller_df.columns
        and not df_spiller_df["position"].empty
        else "Ukendt"
    )
    ...
    if "outcome" not in df_spiller_df.columns:
        ...
        df_spiller_df["outcome"] = pd.to_numeric(
            df_spiller_df["outcome"], errors="coerce"
        ).fillna(0)

    df_filtreret = df_spiller_df[
        ~df_spiller_df["Action_Label"].isin(["Pasning", "Indkast"])
    ].copy()
    ...
    with c_stats_side:
        ...
        st.markdown(
            "<hr style='margin: 10px 0; opacity: 0.5;'>", unsafe_allow_html=True
        )
        ...
        pas_acc = (
            (pas_df["outcome"].eq(1).sum() / pas_count * 100)
            if pas_count > 0
            else 0
        )
        ...
        chancer_skabt = (
            akt_stats[
                akt_stats.index.str.contains(
                    "Key Pass|assist|Stor chance", case=False, na=False
                )
            ]["Total"].sum()
        ...
        shots_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([13, 14, 15, 16])]
        )
        cross_count = len(
            df_spiller_df[
                df_spiller_df["qual_list"].apply(
                    lambda x: "2" in x if isinstance(x, (list, np.ndarray)) else False
                )
            ]
        )
        erob_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([49])]
        )
        touch_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin(TOUCH_IDS)]
        )
        drib_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([3])]
        )
        regains_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([7, 8, 12, 49])]
        )
        boldtab_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([50, 51])]
        )
        def_count = len(
            df_spiller_df[df_spiller_df["event_typeid"].isin([7, 8])]
        )

        if (
            "end_x" in df_spiller_df.columns
            and "event_x" in df_spiller_df.columns
        ):
            fremad_count = len(
                df_spiller_df[
        ...
        st.markdown(
            "<hr style='margin: 15px 0; opacity: 0.5;'>", unsafe_allow_html=True
        )
        ...
        if not akt_stats.empty:
            ...
            for akt, row in akt_stats.head(10).iterrows():
                ...
                stats_html = (
                    ...
                    else f"{succes}/{total} <b>({int(succes/total*100) if total > 0 else 0}%)</b>"
                )
                st.markdown(
    ...
    with c_pitch_side:
        ...
        descriptions_visning = {
            k: v
            for k, v in descriptions.items()
            if k not in skjulte_visninger
        }
        ...
        pitch = Pitch(
            pitch_type="opta", pitch_color="#ffffff", line_color="#BDBDBD"
        )
        fig, ax = pitch.draw(figsize=(10, 7))
        draw_player_info_box(
            ax, hold_logo, valgt_spiller_navn, SEASONNAME, visning
        )
        ...
        if not df_plot.empty:
            if visning == "Heatmap":
                ...
                d = df_plot[
                    df_plot["event_typeid"].isin([5, 7, 8, 12, 49, 55])
                ]
                ax.scatter(
                    d.event_x,
                    d.event_y,
                ...
                if (
                    "end_x" not in df_plot.columns
                    or "end_y" not in df_plot.columns
                ):
                    st.info(
