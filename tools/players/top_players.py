...
# FJERN DENNE LINJE: from tools.players import top_players 
...
def vis_side():
    ...
    # --- CSS: Eksplosive Profiler Layout ---
    st.markdown("""
        <style>
    ...
    # 1. HOLDVALG (Dynamisk key fjerner ID-fejlen)
    ...
    with col_sel:
        valgt_navn = st.selectbox(
            ...
            key="physical_ranking_main_selector" 
        )
    
    target_wyid = TEAMS[valgt_navn]["team_wyid"]

    # 2. SQL: Beregner faktiske RANK mod HELE ligaen
    query = f"""
    WITH LIGA_STATS AS (
        SELECT 
            PLAYER_NAME,
            ...
            for kat_navn, metrics in kategorier.items():
                ...
                for label, rank_col in metrics:
                    ...
                    for i, (_, row) in enumerate(df.iterrows()):
                        ...
                        # Bar-længde: Jo lavere rank (tættere på 1), jo længere bar.
                        # Vi sætter en base på 200 spillere for at skalere baren.
                        fill_width = max(10, (1 - (rank_val / 200)) * 100) if rank_val <= 200 else 10
                        
                        # Farver efter rank: Top 15 = Grøn, Top 50 = Gul, Resten = Rød
                        if rank_val <= 15: color = "#22c55e"
                        elif rank_val <= 50: color = "#facc15"
                        else: color = "#fca5a5"
