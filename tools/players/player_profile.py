...
def vis_side(dp=None):
    ...
    # 2. Hent Sæson-data (Rettet WHERE og Timestamp Casting)
    with st.spinner("Indlæser hele sæsonen..."):
        sql_all_season = f"""
            SELECT 
                e.EVENT_X, 
                e.EVENT_Y, 
                ...
                e.PLAYER_NAME, 
                e.MATCH_OPTAUUID, 
                CAST(e.EVENT_TIMESTAMP AS TIMESTAMP_NTZ) as EVENT_TIMESTAMP, 
    ...
    with t_pitch:
        ...
        valgt_spiller = c1.selectbox("Vælg spiller", spiller_liste, key="prof_psel")
        
        descriptions = {
            "Heatmap": "Bevægelse og intensitet.",
            "Berøringer": "Alle aktioner med boldkontakt.",
            "Afslutninger": "Skudforsøg (Mål = stjerne).",
            "Mål": "Kun scoringer.",
            "Skudassists": "Chancer skabt.",
            "Indlæg": "Indlæg til feltet.",
            "Erobringer": "Opsnappede bolde og tacklinger."
        }
        visning = c2.selectbox("Visning", list(descriptions.keys()), key="prof_vsel")
        c3.caption(descriptions.get(visning))
        ...
        with col_metrics:
            ...
            m_col2.metric("Pasning %", f"{acc}%")

            m_col3, m_col4 = st.columns(2)
            m_col3.metric("Skud", len(df_spiller[df_spiller['EVENT_TYPEID'].isin([13,14,15,16])]))
            m_col4.metric("Erobringer", len(df_spiller[df_spiller['EVENT_TYPEID'].isin([7,8,12,49])]))
            ...
            for akt, row in akt_stats.iterrows():
                ...
                st.markdown(f"""
                    <div style="display:flex; justify-content:space-between; font-size:11px; border-bottom:1px solid #eee; padding:5px 0;">
                        <span>{akt}</span>
        ...
        with col_pitch:
            ...
            if not df_plot.empty:
                if visning == "Heatmap":
                    ...
                    g = df_plot[df_plot['EVENT_TYPEID'] == 16]
                    m = df_plot[df_plot['EVENT_TYPEID'].isin([13, 14, 15])]
                    ax.scatter(m.EVENT_X, m.EVENT_Y, color='red', s=80, edgecolors='black', alpha=0.6, label='Afslutning')
                    ax.scatter(g.EVENT_X, g.EVENT_Y, color='gold', s=150, marker='*', edgecolors='black', zorder=5, label='Mål')
    ...
    with t_phys:
        ...
        st.info("Catapult / GPS integration.")
