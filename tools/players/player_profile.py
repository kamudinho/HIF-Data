...
def vis_side(dp=None):
    ...
    with st.spinner("Henter spillerdata..."):
        sql_events = f"""
            ...
            JOIN (SELECT DISTINCT PLAYER_OPTAUUID, FIRST_NAME, MATCH_NAME FROM {DB}.OPTA_MATCH_LINEUPS WHERE FIRST_NAME IS NOT NULL) p 
        ...
        sql_db_stats = f"""
            ...
            SELECT 
                ...
                g.SHORT_LAST_NAME as short_last_name,
                g.GOALS as goals,
        ...
        if df_phys is None or df_phys.empty:
            ...
            with t_sub_charts:
                ...
                if not df_chart.empty:
                    ...
                    fig.update_layout(
                        title=f"{cat_choice} pr. kamp",
                        xaxis_title="Modstander / Dato",
                        yaxis_title=suffix,
