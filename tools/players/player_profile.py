...
def vis_side(dp=None):
    ...
    with t_pitch:
        if not df_all_h.empty:
            # Styling til metrics: Centreret og fed værdi
            st.markdown("""
                <style>
            ...
            # 1. Top-kontrolbar: Dropdowns til højre
            ...
            with c_stats_side:
                ...
                # --- METRICS: 4 PR. RÆKKE, CENTERET OG FED ---
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("Aktion", total_akt)
                m2.metric("Touch", touch_count)
                m3.metric("Pasn.", pas_count)
                m4.metric("Acc %", f"{int(pas_acc)}%")
                
                m5, m6, m7, m8 = st.columns(4)
                m5.metric("Skud", shots_count)
                m6.metric("Chancer", chancer_skabt)
                m7.metric("Indlæg", cross_count)
                m8.metric("Erob.", erob_count)
                ...
                if not df_filtreret.empty:
                    ...
                    for akt, row in akt_stats.iterrows():
                        ...
                        if akt in bare_antal:
                            ...
                            stats_html = f"{succes}/{total} <span style='font-weight:bold;'>({pct}%)</span>"
