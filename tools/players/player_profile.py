...
def vis_side(dp=None):
    ...
    # DEFINER SPILLER UUID TIL FYSISK DATA
    ...
    with t_phys:
        ...
        if df_phys is not None and not df_phys.empty:
            ...
            st.write(f"**Historik (Seneste kampe) - {valgt_spiller}**")
            st.dataframe(df_phys, use_container_width=True, hide_index=True)
            
            st.write("**Udvikling i High Intensity Runs**")
            st.line_chart(df_phys.set_index('MATCH_DATE')['HI_RUNS'])
        else:
            st.info(f"Ingen fysiske data fundet i Second Spectrum for {valgt_spiller}.")
