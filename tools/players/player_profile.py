...
# --- NY FUNKTION TIL AT TEGNE INFO-BOKSEN PÅ PITCHEN (Baseret på dine gemte værdier) ---
def draw_player_info_box(ax, logo, player_name, season, view_name):
    """Tegner den velkendte mørkeblå info-boks øverst til venstre på banen"""
...
# --- NY FUNKTION TIL AT SKABE RELATIVE DONUT CHARTS (Truppens performance) ---
def create_relative_donut(player_val, max_val, label, color="#003366"):
    """
    Skaber en donut chart, der sammenligner spillerens værdi med holdets maksimum.
    """
    # Base max for at undgå division med 0
...
    # 2. HENT HÆNDELSESDATA FØRST (Løser 'sql is not defined' fejlen)
    sql_all = f"""
           SELECT 
...
    df_spiller = df_all[df_all['VISNINGSNAVN'] == valgt_spiller].copy()
    ...
    # Opret Tabs
    t_profile, t_pitch, t_phys, t_stats, t_compare = st.tabs(["Spillerprofil", "Aktioner på bane", "Fysisk data", "Statistik", "Sammenligning"])

    # --- NY TAB: SPILLERPROFIL (Relativ performance på holdet) ---
    with t_profile:
        # Beregn maksimumsværdier for hele truppen for at sammenligne
...
            raw_erobringer=('EVENT_TYPEID', lambda x: x.isin([7, 8, 12, 49]).sum()),
            raw_afslutninger=('EVENT_TYPEID', lambda x: x.isin([13, 14, 15, 16]).sum())
        )
        
        maks_pas = truppen_stats['raw_pasninger'].max()
        maks_erob = truppen_stats['raw_erobringer'].max()
        maks_skud = truppen_stats['raw_afslutninger'].max()

        # Beregn den valgte spillers værdier
        pasninger_spiller = len(df_spiller[df_spiller['EVENT_TYPEID'] == 1])
        erobringer_spiller = len(df_spiller[df_spiller['EVENT_TYPEID'].isin([7, 8, 12, 49])])
        skud_spiller = len(df_spiller[df_spiller['EVENT_TYPEID'].isin([13, 14, 15, 16])])
...
        with col_r:
            st.markdown("<h4 style='text-align:center;'>Truppens Performance (Relativt til maks)</h4>", unsafe_allow_html=True)
            # Donuts
            r1 = st.columns(3)
            with r1[0]: st.write("Pasninger"); st.plotly_chart(create_relative_donut(pasninger_spiller, maks_pas, "Pasninger"), config={'displayModeBar': False})
            with r1[1]: st.write("Erobringer"); st.plotly_chart(create_relative_donut(erobringer_spiller, maks_erob, "Erobringer", "#cc0000"), config={'displayModeBar': False})
            with r1[2]: st.write("Afslutninger"); st.plotly_chart(create_relative_donut(skud_spiller, maks_skud, "Afslutninger", "#11caa0"), config={'displayModeBar': False})

    # --- TAB: AKTIONER PÅ BANE ( GENETABLERET OG FASTHOLDT ) ---
    with t_pitch:
        descriptions = {
...
                        x=df_chart['Label'], 
                        y=y_vals,
                        text=y_vals.apply(lambda x: f"{x:.1f}" if x < 10 else f"{int(x)}"),
...
                    fig.add_shape(type="line", x0=-0.5, x1=len(df_chart)-0.5, y0=season_avg, y1=season_avg, 
                    ...
                    # Layout optimeret til bar charts over tid
                    fig.update_layout(
                        plot_bgcolor="white", 
                        height=350, 
...
                        yaxis=dict(
                            ...
                            zeroline=False,
                            range=[0, y_vals.max() * 1.15] # Plads til tekst
                        ),
                        showlegend=False
                    )
