...
# --- DE TO ANDRE FANER (selvstændige filer, ingen cirkulær afhængighed) ---
from player_profile import render_spillerprofil
from player_view import render_spilleraktioner
...
def tilfoej_fremadrettede_pasninger(df_stats: pd.DataFrame, df_events: pd.DataFrame) -> pd.DataFrame:
    ...
    (end_x > event_x). Kræver slutkoordinater (end_x/end_y) i data; findes
    de ikke, sættes kolonnen til 0 for alle spillere.
    """
...
def _byg_event_stats(df_events: pd.DataFrame) -> pd.DataFrame:
    """Fælles groupby-beregning for spiller-stats, brugt til både liga, hold og enkeltkamp."""
...
def _agger_expected(df_expected: pd.DataFrame, hold_optauuid: str = None) -> pd.DataFrame:
    ...
    kampe spillet for det pågældende hold.
    """
...
def hent_kampliste(_conn, valgt_uuid_hold: str, seasonname: str) -> pd.DataFrame:
    """Cachet kampliste for det valgte hold - undgår et Snowflake-kald ved hvert rerun."""
    sql_matches = f"""
...
def vis_side(dp=None):
    ...
    t_team, t_matches, t_profile, t_pitch = st.tabs(["Holdoversigt", "Kampoversigt", "Spillerprofil", "Spilleraktioner"])
    ...
    with t_profile:
        render_spillerprofil(truppen_stats, truppen_stats_liga, valgt_player_uuid, valgt_spiller, spiller_position, hold_logo, primær_farve)

    with t_pitch:
        render_spilleraktioner(df_spiller, valgt_spiller, hold_logo, primær_farve, spiller_position, valgt_player_uuid, SEASONNAME)
