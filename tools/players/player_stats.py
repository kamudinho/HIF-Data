...
from data.utils.mapping import get_action_label, har_qualifier, is_assist
...
# --- IMPORT AF SPESIFIKKE HJÆLPERE (HVIS NØDVENDIGT) ---
try:
...
# --- DEFINITION AF KOLONNER ---
GEN_KOLONNER = [
    "visningsnavn", "Kampe", "Minutter", "Aktioner", "Pasninger", 
    "Pasningsprocent", "Mål", "Assists", "Udskiftet", "Indskiftet", "Gule_kort", "Roede_kort"
]
OPB_KOLONNER = [
    "visningsnavn", "Aktioner", "Pasninger", "Pasningsprocent", 
    "Key_Passes", "fremadrettede_pasninger", "Stikninger", "Driblinger", "Driblinger_Succes", "Rum_Driblinger_Space"
]
OFF_KOLONNER = [
...
def formater_tid(row):
    if "formatted_minute" in row and pd.notna(row["formatted_minute"]) and str(row["formatted_minute"]) != "":
        return str(row["formatted_minute"])

    minute, period = row.get("minute"), row.get("period")
    if pd.isna(minute):
        return ""
    ...
    if pd.notna(period):
        try:
            period = int(period)
            if period == 1 and minute > 45: return f"45+{minute - 45}"
            elif period == 2 and minute > 90: return f"90+{minute - 90}"
            elif period == 5 and minute > 105: return f"105+{minute - 105}"
            elif period == 6 and minute > 120: return f"120+{minute - 120}"
        except (ValueError, TypeError):
            pass

    if minute > 90: return f"90+{minute - 90}"
    elif 45 < minute <= 50: return f"45+{minute - 45}"
    return str(minute)
...
def tilfoej_fremadrettede_pasninger(df_stats: pd.DataFrame, df_events: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = (df_events["event_typeid"] == 1) & df_events["end_x"].notna() & (df_events["end_x"] > df_events["event_x"])
...
def _byg_event_stats(df_events: pd.DataFrame) -> pd.DataFrame:
    return (
        df_events.groupby(["player_optauuid", "visningsnavn"])
...
def hent_holdliste(_conn) -> dict:
    ...
    team_map = {}
    if df_teams_raw is not None and not df_teams_raw.empty:
...
def hent_kampliste(_conn, valgt_uuid_hold: str, seasonname: str) -> pd.DataFrame:
    ...
    if df_matches is None: return pd.DataFrame()
...
def _generer_og_vis_tabel(truppen_df, kategori_valg):
    ...
    df_vis = truppen_df.reset_index()
    alle_kategorier = GEN_KOLONNER + OPB_KOLONNER + OFF_KOLONNER + DEF_KOLONNER
    for col in alle_kategorier:
        if col not in df_vis.columns: df_vis[col] = 0

    valgte_kolonner = {
        "Generelt": GEN_KOLONNER,
        "Opbygning": OPB_KOLONNER,
        "Offensiv": OFF_KOLONNER,
        "Defensiv": DEF_KOLONNER
    }.get(kategori_valg, [k for k in df_vis.columns if k != "player_optauuid"])

    eksisterende_kolonner = [k for k in valgte_kolonner if k in df_vis.columns]
    df_visning = df_vis[eksisterende_kolonner].copy()
...
def vis_side():
    ...
    _, col_h_hold = st.columns([3.2, 1.2])
    ...
    with st.spinner("Henter hold- og kampdata..."):
        df_all, truppen_stats, df_expected = byg_holdstats(conn, valgt_uuid_hold, navne_map)
    ...
    with t_team:
        ...
        with col_t_btn:
            ...
            kategori_valg = st.segmented_control("Visningskategori", options=["Generelt", "Opbygning", "Offensiv", "Defensiv"], default="Generelt", key="team_kategori_control", label_visibility="collapsed")
            st.markdown("</div>", unsafe_allow_html=True)

        _generer_og_vis_tabel(truppen_stats, kategori_valg)
    ...
    with t_matches:
        ...
        if not df_matches.empty:
            for _, r in df_matches.iterrows():
                ...
                h_maal = int(r["total_home_score"]) if pd.notna(r["total_home_score"]) else 0
                u_maal = int(r["total_away_score"]) if pd.notna(r["total_away_score"]) else 0
                h_hold, m_mod = (h_maal, u_maal) if er_hjemme else (u_maal, h_maal)
                label = f"Kamp {r['week']}: vs. {modstander} ({h_hold}-{m_mod})"
        ...
        with col_t_btn:
            st.markdown('<div style="display: flex; justify-content: flex-end;">', unsafe_allow_html=True)
            kategori_valg_kamp = st.segmented_control("Visningskategori Kamp", options=["Generelt", "Opbygning", "Offensiv", "Defensiv"], default="Generelt", key="match_kategori_control", label_visibility="collapsed")
            st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div style='margin-bottom: 10px;'></div>", unsafe_allow_html=True)

        if not df_matches.empty and valgt_kamp_uuid:
            match_col = next((col for col in ["match_optauuid", "match_id"] if col in df_all.columns), None)
            df_kamp_events = df_all[df_all[match_col].astype(str) == valgt_kamp_uuid].copy() if match_col else pd.DataFrame()

            if not df_kamp_events.empty:
                truppen_stats_kamp = _byg_event_stats(df_kamp_events)
                
                expected_kamp = df_expected[df_expected["match_optauuid"].astype(str) == str(valgt_kamp_uuid)] if df_expected is not None and not df_expected.empty and "match_optauuid" in df_expected.columns else pd.DataFrame()
                expected_agg_kamp = expected_kamp.groupby("player_optauuid")[["xg", "xa", "minutes"]].sum() if not expected_kamp.empty else pd.DataFrame()

                if not expected_agg_kamp.empty:
                    truppen_stats_kamp["Minutter"] = expected_agg_kamp["minutes"].reindex(truppen_stats_kamp.index, fill_value=0).round(0).astype("Int64")
                    truppen_stats_kamp["xG"] = expected_agg_kamp["xg"].reindex(truppen_stats_kamp.index, fill_value=0).round(2)
                    truppen_stats_kamp["xA"] = expected_agg_kamp["xa"].reindex(truppen_stats_kamp.index, fill_value=0).round(2)
                else:
                    truppen_stats_kamp["Minutter"] = 0
                    truppen_stats_kamp["xG"] = 0.0
                    truppen_stats_kamp["xA"] = 0.0

                truppen_stats_kamp["Mål"] = df_kamp_events[df_kamp_events["event_typeid"] == 16].groupby("player_optauuid").size().reindex(truppen_stats_kamp.index, fill_value=0).astype("Int64")
                truppen_stats_kamp["Assists"] = df_kamp_events.apply(lambda r: 1 if is_assist(r.get("event_typeid"), r.get("qual_list", [])) else 0, axis=1).groupby(df_kamp_events["player_optauuid"]).sum().reindex(truppen_stats_kamp.index, fill_value=0).astype("Int64")
                truppen_stats_kamp["Pasningsprocent"] = ((truppen_stats_kamp["Pasninger_Succes"] / truppen_stats_kamp["Pasninger"]) * 100).where(truppen_stats_kamp["Pasninger"] > 0, 0).round(1)
                truppen_stats_kamp["Position"] = truppen_stats_kamp.index.to_series().apply(lambda u: POSITION_MAP.get(str(u).strip(), "Ukendt"))
                
                truppen_stats_kamp = tilfoej_fremadrettede_pasninger(truppen_stats_kamp, df_kamp_events)

                _generer_og_vis_tabel(truppen_stats_kamp, kategori_valg_kamp)
            else:
