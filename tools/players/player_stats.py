...
def formater_tid(row):
  # Da SQL nu beregner 'formatted_minute' direkte, tager vi den hvis den findes
  if (
      "formatted_minute" in row
      and pd.notna(row["formatted_minute"])
      and str(row["formatted_minute"]) != ""
  ):
...
def tilfoej_kategori_kolonner(
    df_stats: pd.DataFrame, df_events: pd.DataFrame, category_keys
) -> pd.DataFrame:
  ...
  if "qual_set" not in df_events.columns:
    ...
    df_events["qual_set"] = df_events["qual_list"].apply(
        lambda ql: frozenset(str(q).strip() for q in ql)
        if isinstance(ql, list)
  ...
  for key in category_keys:
    ...
    df_stats[key] = (
        counts.reindex(df_stats.index, fill_value=0).astype("Int64")
    )

  return df_stats


def tilfoej_fremadrettede_pasninger(
    df_stats: pd.DataFrame, df_events: pd.DataFrame
) -> pd.DataFrame:
  if (
      df_events is None
      or df_events.empty
      or "end_x" not in df_events.columns
  ):
  ...
  df_stats["fremadrettede_pasninger"] = (
      counts.reindex(df_stats.index, fill_value=0).astype("Int64")
  )
  return df_stats
...
def _byg_event_stats(df_events: pd.DataFrame) -> pd.DataFrame:
  return (
      ...
      .apply(
          lambda x: pd.Series({
              "Kampe": x["match_optauuid"].nunique()
              if "match_optauuid" in x.columns
              else 1,
              "Kamp_Laengde": x["matchlenghtmin"].max()
              if "matchlenghtmin" in x.columns
              else 90,
              "Aktioner": len(x),
              "Gule_kort": df_events[df_events["event_typeid"] == 17]
              .apply(lambda r: 1 if "31" in _get_quals(r) else 0, axis=1)
              .sum(),
              "Roede_kort": df_events[df_events["event_typeid"] == 17]
              .apply(lambda r: 1 if "33" in _get_quals(r) else 0, axis=1)
              .sum(),
              "Indskiftet": (x["event_typeid"] == 19).sum(),
              "Udskiftet": (x["event_typeid"] == 18).sum(),
              "Pasninger": x["Pasninger_Total"].sum()
              if "Pasninger_Total" in x.columns
              else 0,
              "Pasninger_Succes": x["Pasninger_Succes"].sum()
              if "Pasninger_Succes" in x.columns
              else 0,
              ...
              "Driblinger_Succes": x.apply(
                  lambda r: 1
                  if str(r["event_typeid"]) == "3" and "211" not in _get_quals(r)
                  else 0,
                  axis=1,
              ).sum(),
              "Gennembrud_Overtake": x.apply(
                  lambda r: 1
                  if str(r["event_typeid"]) == "3" and "465" in _get_quals(r)
                  else 0,
                  axis=1,
              ).sum(),
              "Rum_Driblinger_Space": x.apply(
                  lambda r: 1
                  if str(r["event_typeid"]) == "3" and "464" in _get_quals(r)
                  else 0,
                  axis=1,
              ).sum(),
              "Offensive_Dueller": x.apply(
                  lambda r: 1 if "286" in _get_quals(r) else 0, axis=1
              ).sum(),
              "Defensive_Dueller": x.apply(
                  lambda r: 1 if "285" in _get_quals(r) else 0, axis=1
              ).sum(),
              "Defensive_1v1_Stoppet": x.apply(
                  lambda r: 1 if "467" in _get_quals(r) else 0, axis=1
              ).sum(),
              "Chancer_skabt": x.apply(
                  lambda r: 1 if "210" in _get_quals(r) else 0, axis=1
              ).sum(),
              "Key_Passes": x.apply(
                  lambda r: 1 if "210" in _get_quals(r) else 0, axis=1
              ).sum(),
...
def _forbered_events(df: pd.DataFrame) -> pd.DataFrame:
  ...
  df["Pasninger_Succes"] = (
      (df["event_typeid"] == 1) & (df["outcome"] == 1)
  ).astype(int)
...
def _agger_expected(
    df_expected: pd.DataFrame, hold_optauuid: str = None
) -> pd.DataFrame:
...
def hent_navne_map() -> dict:
  try:
    csv_path = os.path.join(
        os.getcwd(), "data", "players", "1div_overskrivning.csv"
    )
...
def hent_kampliste(
    _conn, valgt_uuid_hold: str, seasonname: str
) -> pd.DataFrame:
  sql_matches = f"""
...
def byg_holdstats(_conn, valgt_uuid_hold: str, navne_map: dict):
  df_all_raw, df_expected, _ = hent_match_og_haendelsesdata(
  ...
  truppen_stats["Minutter"] = (
      expected_hold["minutes"]
      .reindex(truppen_stats.index, fill_value=0)
      .round(0)
      .astype("Int64")
  )
  truppen_stats["xG"] = (
      expected_hold["xg"].reindex(truppen_stats.index, fill_value=0).round(2)
  )
  truppen_stats["xA"] = (
      expected_hold["xa"].reindex(truppen_stats.index, fill_value=0).round(2)
  )
  truppen_stats["Mål"] = (
      df_all[df_all["event_typeid"] == 16]
      .groupby("player_optauuid")
      .size()
      .reindex(truppen_stats.index, fill_value=0)
      .astype("Int64")
  )
  truppen_stats["Assists"] = (
      df_all.apply(
          lambda r: 1
          if is_assist(r.get("event_typeid"), r.get("qual_list", []))
          else 0,
          axis=1,
      )
      .groupby(df_all["player_optauuid"])
      .sum()
      .reindex(truppen_stats.index, fill_value=0)
      .astype("Int64")
  )
  truppen_stats["Pasningsprocent"] = (
      (
          truppen_stats["Pasninger_Succes"]
          / truppen_stats["Pasninger"]
          * 100
      )
      .where(truppen_stats["Pasninger"] > 0, 0)
      .round(1)
  )
  ...
  truppen_stats = tilfoej_kategori_kolonner(
      truppen_stats, df_all, ALLE_SPQ_KATEGORIER
  )
  ...
  if df_expected is not None and not df_expected.empty:
    ...
    for col in ("xg", "xa", "minutes"):
      if col in df_expected.columns:
        df_expected[col] = pd.to_numeric(
            df_expected[col], errors="coerce"
        ).fillna(0)
...
def vis_side():
  ...
  with st.spinner("Henter hold- og kampdata..."):
    df_all, truppen_stats, df_expected = byg_holdstats(
        conn, valgt_uuid_hold, navne_map
    )
  ...
  with t_team:
    ...
    with col_t_title:
      ...
      st.markdown(
          f'<div style="display: flex; align-items: center; padding-top:'
          f' 20px;">{logo_html}<span style="font-size: 16px; font-weight:'
          f' bold; line-height: 1;">{valgt_hold.upper()}</span></div>',
          unsafe_allow_html=True,
      )
    ...
    if not truppen_stats.empty:
      ...
      for col in (
          gen_kolonner + opb_kolonner + off_kolonner + def_kolonner
      ):
      ...
      if kategori_valg == "Generelt":
        eksisterende_kolonner = [
            k for k in gen_kolonner if k in df_vis_truppen.columns
        ]
      elif kategori_valg == "Opbygning":
        eksisterende_kolonner = [
            k for k in opb_kolonner if k in df_vis_truppen.columns
        ]
      elif kategori_valg == "Offensiv":
        eksisterende_kolonner = [
            k for k in off_kolonner if k in df_vis_truppen.columns
        ]
      elif kategori_valg == "Defensiv":
        eksisterende_kolonner = [
            k for k in def_kolonner if k in df_vis_truppen.columns
        ]
      else:
        eksisterende_kolonner = [
            k for k in df_vis_truppen.columns if k != "player_optauuid"
        ]
  ...
  with t_matches:
    ...
    with col_t_title:
      ...
      st.markdown(
          ...
          f' items;>{logo_html}<span style="font-size: 16px; font-weight:'
          ' bold; line-height: 1;">KAMPOVERSIGT</span></div>',
          unsafe_allow_html=True,
      )
    ...
    if not df_matches.empty and valgt_kamp_uuid:
      ...
      df_kamp_events = (
          df_all[df_all[match_col_in_all].astype(str) == valgt_kamp_uuid].copy()
          if match_col_in_all
          else pd.DataFrame()
      )
      ...
      if not df_kamp_events.empty:
        ...
        if (
            df_expected is not None
            and not df_expected.empty
            and "match_optauuid" in df_expected.columns
        ):
          df_expected_kamp = df_expected[
              df_expected["match_optauuid"].astype(str) == str(valgt_kamp_uuid)
          ]
          if not df_expected_kamp.empty:
            expected_agg_kamp = df_expected_kamp.groupby("player_optauuid")[
                ["xg", "xa", "minutes"]
            ].sum()

        if not expected_agg_kamp.empty:
          truppen_stats_kamp_raw["Minutter"] = (
              expected_agg_kamp["minutes"]
              .reindex(truppen_stats_kamp_raw.index, fill_value=0)
              .round(0)
              .astype("Int64")
          )
          truppen_stats_kamp_raw["xG"] = (
              expected_agg_kamp["xg"]
              .reindex(truppen_stats_kamp_raw.index, fill_value=0)
              .round(2)
          )
          truppen_stats_kamp_raw["xA"] = (
              expected_agg_kamp["xa"]
              .reindex(truppen_stats_kamp_raw.index, fill_value=0)
              .round(2)
          )
        else:
        ...
        truppen_stats_kamp_raw["Mål"] = (
            df_kamp_events[df_kamp_events["event_typeid"] == 16]
            .groupby("player_optauuid")
            .size()
            .reindex(truppen_stats_kamp_raw.index, fill_value=0)
            .astype("Int64")
        )
        truppen_stats_kamp_raw["Assists"] = (
            df_kamp_events.apply(
                lambda r: 1
                if is_assist(r.get("event_typeid"), r.get("qual_list", []))
                else 0,
                axis=1,
            )
            .groupby(df_kamp_events["player_optauuid"])
            .sum()
            .reindex(truppen_stats_kamp_raw.index, fill_value=0)
            .astype("Int64")
        )
        ...
        truppen_stats_kamp_kamp["Pasningsprocent"] = (
            ...
            .where(truppen_stats_kamp_kamp["Pasninger"] > 0, 0)
            .round(1)
        )

        truppen_stats_kamp_kamp["Position"] = (
            truppen_stats_kamp_kamp.index.to_series().apply(
                lambda u: POSITION_MAP.get(str(u).strip(), "Ukendt")
            )
        )
        truppen_stats_kamp_kamp = tilfoej_fremadrettede_pasninger(
            truppen_stats_kamp_kamp, df_kamp_events
        )
