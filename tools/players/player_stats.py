...
from data.utils.mapping import get_action_label, is_assist, har_qualifier
...
_STATIC_PLAYERS = getattr(player_mapping, 'PLAYER_MAPPING', []) if 'player_mapping' in globals() else []
POSITION_MAP = {
...
@st.cache_data(ttl=300, show_spinner="Behandler holddata...")
...
def vis_side():
    ...
    # Top-kontrol til at vælge hold
    col_spacer_top, col_h_hold = st.columns([3, 1])
    ...
    with st.spinner("Henter holdoversigt..."):
        df_all, truppen_stats = byg_holdstats(conn, valgt_uuid_hold, navne_map)

    if df_all.empty:
        st.warning("Ingen hændelsesdata fundet for holdet.")
        st.stop()

    # --- HOLDOVERSIGT SEKTION ---
