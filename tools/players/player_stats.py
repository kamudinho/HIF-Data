...
# ---------------------------------------------------------------------------
# FÆLLES SIDE-OPSÆTNING
# ---------------------------------------------------------------------------
def hent_side_kontekst():
    ...
    valgt_label = col_h_spiller.selectbox(
        "Spiller", spiller_liste if spiller_liste else [""],
        label_visibility="collapsed", key="global_spiller_select"
    )
    ...
    return {
        'conn': conn,
        'valgt_hold': valgt_hold,
        'valgt_uuid_hold': valgt_uuid_hold,
        'hold_logo': hold_logo,
        ...
        'df_all': df_all,
        'df_liga_total': df_liga_total,
        'truppen_stats': truppen_stats,
        'truppen_stats_liga': truppen_stats_liga,
...
def vis_side(dp=None):
    """Renderer denne sides indhold: Holdoversigt + Kampoversigt."""
    ctx = hent_side_kontekst()
    if ctx is None:
        return

    valgt_hold = ctx['valgt_hold']
    valgt_uuid_hold = ctx['valgt_uuid_hold']
    hold_logo = ctx['hold_logo']
    truppen_stats = ctx['truppen_stats']
    df_all = ctx['df_all']
    df_expected = ctx['df_expected']
    conn = ctx['conn']
