...
from data.data_load import _get_snowflake_conn  # Tilbage til din fungerende forbindelse
...
def query_db(sql):
    """Hurtig hjælper til at køre queries via din eksisterende forbindelse"""
...
def vis_side():
    # CSS: Styling til flade hvide tabs med rød bundlinje
    st.markdown("""
        <style>
    ...
    # Dropdowns (Husk caching af disse for fart)
    ...
    if not df.empty:
        ...
        # Håndtering af minutter (hvis det er streng eller tal)
        ...
        with t_int:
            # Hent kun F53A når denne fane er aktiv
