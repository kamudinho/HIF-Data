...
def get_extended_player_data(player_name, player_opta_uuid, target_team_ssiid, _conn):
    """Henter udvidet fysisk data - sikret mod NaN og forkerte klubber"""
    clean_id = str(player_opta_uuid).lower().replace('p', '').strip()
    # Vi bruger optaId som primær nøgle for at undgå navne-mismatch
    ...
    sql = f"""
        SELECT 
            ...
            -- Vi tvinger NULL til 0 direkte i SQL for at undgå NaN
        ...
        LEFT JOIN {DB}.SECONDSPECTRUM_F53A_GAME_PLAYER f 
            ON s.MATCH_SSIID = f.MATCH_SSIID AND (s.PLAYER_NAME = f.PLAYER_NAME OR s."optaId" = f."optaId")
        LEFT JOIN {DB}.SECONDSPECTRUM_SEASON_METADATA sea
            ON s.MATCH_SSIID = sea.MATCH_SSIID
        WHERE (s."optaId" = '{clean_id}' OR s.PLAYER_NAME ILIKE '%{player_name.split(' ')[-1]}%')
...
def get_minute_splits(match_ssiid, player_name, _conn):
    sql = f"""
        ...
        WHERE MATCH_SSIID = '{match_ssiid}' 
          AND (PLAYER_NAME = '{player_name}' OR PLAYER_NAME ILIKE '%{player_name.split(' ')[-1]}%')
...
def vis_side():
    ...
    if df is not None and not df.empty:
        df = df.fillna(0) # GLOBAL FIX: Erstat alle NaN med 0
        latest = df.iloc[0]
        ...
        # ALLE TABS ER TILBAGE
        tabs = st.tabs(["Fase-overblik", "Intensitets Profil", "Minut Splits", "Sæson Trend", "Kamp Data"])

        with tabs[0]:
            st.write(f"### HSR Fordeling: {latest['MATCH_TEAMS']} ({latest['MATCH_DATE']})")
            c_a, c_b = st.columns(2)
            c_a.pyplot(draw_phase_pitch(latest['HSR_TIP'], "TIP (Angreb)", "#2ecc71"))
            c_b.pyplot(draw_phase_pitch(latest['HSR_OTIP'], "OTIP (Forsvar)", "#e74c3c"))
        ...
        with tabs[1]:
            ...
            fig_bar = go.Figure(go.Bar(
                ...
                text=[f"{v}%" for v in z_vals], textposition='outside'
            ))
            fig_bar.update_layout(xaxis=dict(range=[0, max(z_vals)+15]), height=400, margin=dict(t=20, b=20))
        ...
        with tabs[2]:
            ...
            if not df_splits.empty:
                ...
                fig_split = go.Figure()
                fig_split.add_trace(go.Scatter(x=df_hsr_splits['MINUTE_SPLIT'], y=df_hsr_splits['PHYSICAL_METRIC_VALUE'], fill='tozeroy', line_color='#cc0000'))
                st.plotly_chart(fig_split, use_container_width=True)
            else:
                st.info("Ingen split-data tilgængelig.")

        with tabs[3]:
            df_trend = df.sort_values('MATCH_DATE')
            fig_trend = go.Figure()
            fig_trend.add_trace(go.Scatter(x=df_trend['MATCH_DATE'], y=df_trend['HSR'], name="HSR", line=dict(color='#cc0000', width=3)))
