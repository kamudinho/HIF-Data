...
def get_extended_player_data(player_name, player_opta_uuid, target_team_ssiid, _conn):
    ...
    # Vi bruger efternavnet som fallback hvis navnet er forskelligt i tabellerne
    ...
    sql = f"""
        SELECT 
            ...
            -- Vi tvinger NULL til 0 for at undgå NaN fejl i Streamlit
        ...
        LEFT JOIN {DB}.SECONDSPECTRUM_F53A_GAME_PLAYER f 
            ...
            AND (s.PLAYER_NAME = f.PLAYER_NAME OR f.PLAYER_NAME ILIKE '%{last_name}%')
        WHERE (s."optaId" = '{clean_id}' OR s.PLAYER_NAME = '{player_name}')
...
def get_minute_splits(match_ssiid, player_name, _conn):
    ...
    sql = f"""
        ...
        WHERE MATCH_SSIID = '{match_ssiid}' 
          AND (PLAYER_NAME = '{player_name}' OR PLAYER_NAME ILIKE '%{last_name}%')
...
def vis_side():
    ...
    if df is not None and not df.empty:
        ...
        st.subheader(f"Kamp: {latest['MATCH_TEAMS']} ({latest['MATCH_DATE']})")
        ...
        with tabs[1]:
            ...
            fig.update_layout(xaxis=dict(range=[0, 70]), height=400, margin=dict(t=20, b=20))
        ...
        with tabs[2]:
            ...
            if not df_splits.empty:
                ...
                fig_s.update_layout(height=400, xaxis_title="Minut", yaxis_title="HSR Meter")
                st.plotly_chart(fig_s, use_container_width=True)

        with tabs[3]:
            df_trend = df.sort_values('MATCH_DATE')
            fig_t = go.Figure()
            fig_t.add_trace(go.Scatter(x=df_trend['MATCH_DATE'], y=df_trend['HSR'], name="HSR Distance", line=dict(color='#cc0000', width=3)))
