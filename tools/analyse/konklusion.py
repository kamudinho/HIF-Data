...
def vis_side(dp=None):
    # --- 1. CONFIG & DATA ---
    # Vi henter værdier fra din mapping-fil
    ...
    for col in ['TOTAL_GOALS', 'OP_GOALS', 'TOTAL_SHOTS', 'TOTAL_XG', 'AVG_POSS']:
        if col in df_league.columns:
            df_league[col] = df_league[col].astype(float)
    ...
    # Mapping af hold fra din konfiguration
    liga_hold = {n: i.get("opta_uuid") for n, i in TEAMS.items() if i.get("league") == COMPETITION_NAME}
    
    # Standardvalget er Hvidovre (ID 7490)
    hif_default = "Hvidovre" if "Hvidovre" in liga_hold else sorted(liga_hold.keys())[0]
    valgt_hold = st.selectbox("Vælg hold", sorted(liga_hold.keys()), index=sorted(liga_hold.keys()).index(hif_default))
    target_uuid = liga_hold[valgt_hold]

    # Beregn metrics nu hvor typerne er ens
    ...
    st.markdown(f"<span style='color:{conc_color}; font-weight:bold;'>Conclusion – {finish_text}</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"* **24th** for percentage of shots taken outside the box (27%)")
    st.markdown(f"<span style='color:{conc_color}; font-weight:bold;'>Conclusion – prefer high quality chances</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span style='color:{conc_color}; font-weight:bold;'>Conclusion – strong passing retention</span>", unsafe_allow_html=True)
