...
def get_stat_card(label, value, color):
    # Kompakt stat-kort med stribe i bunden
    return f"""
        <div style="background-color: #f8f9fa; padding: 5px; border-radius: 4px; 
                    border-bottom: 3px solid {color}; width: 55px; text-align: center; margin-right: 4px; margin-bottom: 4px;">
            <div style="font-size: 0.5rem; text-transform: uppercase; color: #666; font-weight: bold;">{label}</div>
            <div style="font-size: 0.9rem; font-weight: 800; color: #1a1a1a;">{value}</div>
        </div>
    """
...
def vis_side(df_spillere, d1, d2, career_df, d3):
    ...
    # --- SELVE LAYOUTET ---
    # Struktur: [Billede P1] [Navn/Stats P1] [RADAR] [Navn/Stats P2] [Billede P2]
    col_img1, col_data1, col_radar, col_data2, col_img2 = st.columns([1.2, 2.5, 4.5, 2.5, 1.2])
    ...
    with col_data1:
        ...
        st.markdown(f"<p style='margin:0 0 8px 0; font-size:0.75rem; color:gray;'>{p1['pos']} | {p1['klub']}</p>", unsafe_allow_html=True)
        # Samler stats i én markdown for at undgå at Streamlit klipper dem over
        stats_html1 = "".join([get_stat_card(k, v, HIF_RED) for k, v in p1['stats'].items()])
        st.markdown(f"<div style='display: flex; flex-wrap: wrap;'>{stats_html1}</div>", unsafe_allow_html=True)
    ...
    with col_data2:
        ...
        st.markdown(f"<p style='margin:0 0 8px 0; font-size:0.75rem; color:gray; text-align:right;'>{p2['pos']} | {p2['klub']}</p>", unsafe_allow_html=True)
        stats_html2 = "".join([get_stat_card(k, v, HIF_BLUE) for k, v in p2['stats'].items()])
        st.markdown(f"<div style='display: flex; flex-wrap: wrap; justify-content: flex-end;'>{stats_html2}</div>", unsafe_allow_html=True)
