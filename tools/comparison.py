...
def vis_side(spillere, player_events, df_scout):
    ...
    def hent_info(navn):
        ...
        return raw_id, klub, pos, stats_data, tech, scout_texts

    id1, k1, pos1, st1, t1, txt1 = hent_info(s1_navn)
    id2, k2, pos2, st2, t2, txt2 = hent_info(s2_navn)

    # --- SIKKER BILLEDFUNKTION (DIN STANDARD) ---
    def vis_spiller_billede(pid, w=110):
        ...
        try:
            # Vi tjekker hurtigt om billedet findes
    ...
    with col2:
        ...
        fig = go.Figure()
        r1 = [t1[k] for k in t1.keys()]; r1.append(r1[0])
        r2 = [t2[k] for k in t2.keys()]; r2.append(r2[0])
