...
def vis_side(spillere, player_events, df_scout):
    ...
    def hent_spiller_data(navn):
        try:
            ...
            search_id = str(int(float(p_id))) if pd.notna(p_id) and str(p_id) != "0" else "0"
            ...
            stats = stats_match.iloc[0].to_dict() if not stats_match.empty and search_id != "0" else {}
            ...
            if not scout_match.empty and search_id != "0":
    ...
    def vis_metrics(row, scout_info, navn, color, side):
        ...
        # Navne-sektion med billede (Symmetrisk layout)
        if side == "venstre":
