...
# --- 2. MASTER MATCHER (Filtreret på Turnering) ---
...
# --- 3. HOVEDFUNKTION (KOMPAKT LAYOUT) ---
def vis_side(analysis_package=None):
    ...
    # FIGSIZE=(3, 4) er nøglen til at de ikke fylder hele skærmen!
    ...
    with tabs[0]:
        ...
        for col, phase, title, dot_c in zip([c1, c2], ['inPossession', 'outOfPossession'], ["OFFENSIV", "DEFENSIV"], [t_color, "#333333"]):
            ...
            with col:
                ...
                if not data.empty:
                    pitch.scatter(data.averageRolePositionX, data.averageRolePositionY, s=150, color=dot_c, edgecolors='black', linewidth=0.6, ax=ax, zorder=3)
                    for _, row in data.iterrows():
                        pitch.annotate(int(row.shirtNumber), (row.averageRolePositionX, row.averageRolePositionY), color='white', va='center', ha='center', fontsize=6, fontweight='bold', ax=ax, zorder=4)
    ...
    with tabs[3]:
        if not df_events.empty:
            ...
            metrics = [([1], 'Passes'), ([4,5], 'Duels'), ([8,49], 'Recovers')]
