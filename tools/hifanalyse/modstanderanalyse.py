...
# --- 1. HJÆLPEFUNKTIONER (OPTA FOCOUS) ---
...
def get_team_style(team_name):
    """Henter farver og logo baseret på holdnavn fra Opta-data"""
    ...
    # Farve-match (tjekker mod din team_mapping.py)
...
def draw_shape_pitch(df_shape, title, color, logo):
    ...
    if not df_shape.empty:
        try:
            # Vi bruger SHAPE_TIMEINSHAPE til at finde den mest relevante taktiske opstilling
        ...
        except: ax.text(50, 50, "Opta Shape Data Format Fejl", ha='center')
    st.markdown(f'<p class="pitch-label" style="text-align:center;font-weight:bold;">{title}</p>', unsafe_allow_html=True)
...
def vis_side(analysis_package=None):
    if not analysis_package:
        st.error("Ingen Opta-datapakke modtaget.")
        return

    # Sørg for at alle Snowflake-tabeller er i UPPERCASE
    ...
    if df_matches.empty:
        st.warning("⚠️ Ingen Opta-kampdata fundet.")
        return

    # Holdvælger (Baseret på Opta Contestant navne)
    ...
    # Find den unikke OPTA UUID for det valgte hold
    ...
    with tabs[0]: # STRUKTUR (Opta Shapes)
        c1, c2 = st.columns(2)
    ...
    with tabs[1]: # OFFENSIV (Opta Heatmaps)
        if not df_hold_ev.empty:
            ...
            for col, title, x_lims in zip([c1, c2], ["OPBYGNING", "AFSLUTNING"], [(0, 50), (50, 100)]):
                with col:
                    st.markdown(f'<p class="pitch-label" style="text-align:center;font-weight:bold;">{title}</p>', unsafe_allow_html=True)
                    fig, ax = pitch.draw(figsize=(6, 8)); ax.set_ylim(x_lims)
    ...
    with tabs[2]: # DEFENSIV (Opta Def. Actions)
        if not df_hold_ev.empty:
            pitch = VerticalPitch(pitch_type='opta', pitch_color='#ffffff', line_color='#333333')
            c1, c2 = st.columns(2)
            ...
            for i, (tid, nav) in enumerate(metrics):
                with cols[i]:
                    ...
                    for name, val in top.items():
                        st.markdown(f'<div style="background-color:#f8f9fa;padding:8px;border-radius:6px;border-left:4px solid {t_color};margin-bottom:5px;font-size:0.85rem;"><b>{val}</b> {name}</div>', unsafe_allow_html=True)
