...
def vis_side(dp):
    ...
    # Rens data og fjern 0,0-hændelser (Buur-fejlen)
    ...
    with col_side:
        ...
        # Roller: Scorer og Sidst på bolden
    ...
    with col_main:
        ...
        for i, r in active_seq.iterrows():
            ...
            # --- LOGIK: FJERN DOBBELT-PRIK VED INDKAST ---
            # Hvis det er et indkast (Type 5), men den næste aktion foregår præcis samme sted (x, y),
            # så springer vi den første over for at undgå to spillere på sidelinjen.
            if i < len(active_seq)-1:
                next_r = active_seq.iloc[i+1]
                if r['EVENT_TYPEID'] == 5 and r['RAW_X'] == next_r['RAW_X'] and r['RAW_Y'] == next_r['RAW_Y']:
                    continue

            # Vis kun HIF eller relevante modstander-aktioner
            is_relevant = is_hif or (not is_hif and r['EVENT_TYPEID'] in [12, 13, 14, 15, 43, 44])
            
            if is_relevant:
                rx, ry = r['RAW_X'], r['RAW_Y']
                # Universal alignment: Tving alt til angrebsenden
        ...
        # Tegn prikker (HIF = Store/Farvede, Modstander = Små/Sorte)
