...
def vis_side(dp):
    # 1. Genindfør Stat-bokse og styling
    st.markdown(f"""
        <style>
    ...
    with col_main:
        ...
        # Den vigtige flip-logik
        should_flip = True if sel_row['RAW_X'] < 50 else False
        
        # --- NY LOGIK: Tegn alt baseret på hvor BOLDEN er ---
        processed_coords = []
        for i, r in active_seq.iterrows():
            is_hif = r['EVENT_CONTESTANT_OPTAUUID'] == HIF_UUID
            
            # 1. Beregn grund-koordinater
            raw_x, raw_y = r['RAW_X'], r['RAW_Y']
            
            # 2. Hvis vi flipper for HIF, flipper vi kun modstanderen hvis han er "spejlvendt" i dataen
            # I en duel skal de have samme koordinat. 
            cx = 100 - raw_x if should_flip else raw_x
            cy = 100 - raw_y if should_flip else raw_y
            
            processed_coords.append((cx, cy, is_hif))
            ...
            if is_hif:
                ...
                m_c = 'black' # Modstander (nu placeret korrekt ved bolden)
                z = 2
            
            pitch.scatter(cx, cy, s=180 if is_hif else 100, color=m_c, edgecolors='white', ax=ax, zorder=z)

        # Tegn stregerne bagefter for at sikre de kun går kronologisk
        for i in range(1, len(processed_coords)):
            cx, cy, is_hif = processed_coords[i]
            px, py, prev_hif = processed_coords[i-1]
            
            # TEGN KUN stregen hvis den går TIL en HIF-spiller
            # Dette fjerner edderkoppe-benene til de sorte prikker
            if is_hif:
        ...
        # TILFØJ TABEL FOR AT TJEKKE DATA (Valgfrit, men godt til fejlsøgning)
        st.write("Sekvens-log (tjek koordinater her):")
