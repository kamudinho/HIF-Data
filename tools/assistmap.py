...
def vis_side(dp):
    # CSS til styling af den manuelle tabel og stat-bokse
    st.markdown("""
        <style>
    ...
    # --- TAB 1: SPILLEROVERSIGT (MANUEL HTML FOR AT UNDGÅ SCROLL) ---
    with tab1:
        ...
        # Generer HTML Tabel manuelt for 100% kontrol
        ...
        for _, row in df_table.iterrows():
            table_html += f"""
                <tr>
                    <td><b>{row[player_col]}</b></td>
                    <td>{row['Assists']}</td>
                    <td>{row['Key_Passes']}</td>
                    <td>{row['Corner_Assists']}</td>
                    <td>{row['Cross_Assists']}</td>
    ...
    with tab2:
        ...
        with col_ctrl_a:
            ...
            if not df_filtered.empty and 'GOAL_SCORER' in df_filtered.columns:
                ...
                # Tæller både målscorere og dem der "bare" afsluttede (Key Pass modtagere)
        ...
        with col_viz_a:
            ...
            if not df_filtered.empty:
                ...
                # Goal Assists (Guld pile + Cirkler)
