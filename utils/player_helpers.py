...
def get_logo_img(opta_uuid):
    ...
    url = next(
        (info["logo"] for name, info in TEAMS.items() 
         if str(info.get("opta_uuid", "")).lower().replace("t", "") == uuid_clean),
        None
    )
    if not url:
        return None
...
def oversæt_qualifiers(qualifier_list_str):
    ...
    if not qualifier_list_str or not isinstance(qualifier_list_str, str):
        return ""
    mapping = {
        "56": "Lang bold", "140": "Hovedstød", "9": "Fod", 
        "210": "Indeni feltet", "212": "Langskud"
    }
