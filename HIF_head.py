...
def vis_side(dp=None):
    ...
    # Opdel i HIF kampe (til S-U-N) og find næste modstander
    ...
    played_hif = hif_m[is_played]

    # --- BEREGN HIF STATUS (S-U-N) ---
    summary = {"S": 0, "U": 0, "N": 0}
    for _, m in played_hif.iterrows():
    ...
    # KOLONNE 1: NÆSTE KAMP & MODSTANDER FORM
    with col1:
        st.caption("##### Næste Kamp & Modstander Form")
        with st.container(border=True):
            if not future.empty:
                ...
                st.markdown(f"**{opp_name}** ({'H' if er_hjemme else 'U'})")
                st.caption(f"{nk['MATCH_DATE_FULL'].strftime('%d. %b')} | Runde {int(nk['WEEK'])}")
                
                # MODSTANDERENS FORM (Legends)
                st.markdown("<div style='margin-top:8px; border-top:1px solid #eee; padding-top:8px;'></div>", unsafe_allow_html=True)
                ...
                if not opp_matches.empty:
                    f_cols = st.columns(5)
                    # Vi vender dem om (head(5).iloc[::-1]) så den nyeste er til højre (eller venstre alt efter præference)
                    for i, (_, m) in enumerate(opp_matches.iloc[::-1].iterrows()):
                ...
                st.caption(f"HIF Status: {summary['S']}V-{summary['U']}U-{summary['N']}T")
            else:
    ...
    with col3:
        ...
        with st.container(border=True):
            try:
                ...
                for _, r in df_e.tail(8).iterrows():
