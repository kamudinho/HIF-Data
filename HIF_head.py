...
def vis_side():
    ...
    st.markdown("### Holdets Nøgletal - Sæson 2026/2027")
    
    # Vi simulerer eller henter holdets overordnede data for sæsonen
    # (Du kan her erstatte med et rigtigt kald til din datakilde, f.eks. hent_hold_stats())
    ...
    with col_left:
        st.markdown("#### Seneste 5 kampe")
        # Eksempel på en simpel, ren tabel over de seneste kampe
        recent_matches = pd.DataFrame({
            "Kamp": ["Hvidovre - AC Horsens", "Vendsyssel - Hvidovre", "Hvidovre - Esbjerg fB", "Kolding IF - Hvidovre", "Hvidovre - FC Fredericia"],
    ...
    with col_right:
        ...
        # Eksempel på holdets dybdegående statistikker
        tactical_stats = pd.DataFrame({
            "Parameter": ["PPDA (Pres-intensitet)", "Afslutninger pr. kamp", "Skud imod pr. kamp", "Dueller vundet", "Succesfulde afleveringer"],
    ...
    st.markdown("### Stilling i NordicBet Liga")
    
    # Eksempel på stillingstabel
