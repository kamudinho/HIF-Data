...
def apply_custom_style():
    st.markdown("""
        <style>
            ...
            .stats-table { width: 100%; font-size: 12px; border-collapse: collapse; table-layout: fixed; }
...
def get_opta_queries(liga_f, saeson_f, hif_only=False):
    ...
    return {"opta_team_stats": f"""
        ...
        msh.POSSESSION AS HOME_POSS, msh.TOTAL_PASSES AS HOME_PASSES, msh.CORNERS AS HOME_CORNERS, msh.CROSSES AS HOME_CROSSES, msh.FORMATION AS HOME_FORMATION, 
        fp_h.FORWARD_PASSES AS HOME_FORWARD_PASSES, 
        sa.XG AS AWAY_XG, sa.SHOTS AS AWAY_SHOTS, sa.TOUCHES_IN_BOX AS AWAY_TOUCHES, 
        msa.POSSESSION AS AWAY_POSS, msa.TOTAL_PASSES AS AWAY_PASSES, msa.CORNERS AS AWAY_CORNERS, msa.CROSSES AS AWAY_CROSSES, msa.FORMATION AS AWAY_FORMATION, 
