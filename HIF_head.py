...
def vis_side():
    ...
    # Henter queries dynamisk (Vi trækker hele ligaen, så vi kan regne på både HIF og modstander)
    queries = get_opta_queries("NordicBet Liga", "2025/2026", hif_only=False)
    
    # Kør Snowflake forespørgsler
    ...
    # Sikrer korrekt dato-formatering til filtrering
...
# --- NY RÆKKE: TRENDLINES (6 kolonner) ---
    ...
    for i, col in enumerate(trend_cols):
        with col:
            ...
            # Possession fix: Vi definerer domænet explicit
            ...
            chart = alt.Chart(hif_recent).mark_line(color='#111', strokeWidth=2).encode(
                x=alt.X('index:Q', axis=alt.Axis(
                    domain=True, grid=False, labels=False, ticks=False, title=None
